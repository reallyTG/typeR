...
### Name: recur.pre.bart
### Title: Data construction for recurrent events with BART
### Aliases: recur.pre.bart
...
## Not run: 
##D ## set.seed(99)
##D ## post <- recur.bart(y.train=pre$y.train, x.train=pre$tx.train, x.test=pre$tx.test)
##D ## depending on your performance, you may want to run in parallel if available
##D post <- mc.recur.bart(y.train=pre$y.train, x.train=pre$tx.train,
##D                       x.test=pre$tx.test, mc.cores=8, seed=99)
##D 
##D M <- nrow(post$yhat.test)
##D RI.B6.Pl <- matrix(0, nrow=M, ncol=K)
##D RI.Th.Pl <- matrix(0, nrow=M, ncol=K)
##D RI.Th.B6 <- matrix(0, nrow=M, ncol=K)
##D 
##D for(j in 1:K) {
##D     h <- seq(j, NK, K)
##D     RI.B6.Pl[ , j] <- apply(post$prob.test[ , h+NK]/
##D                             post$prob.test[ , h], 1, mean)
##D     RI.Th.Pl[ , j] <- apply(post$prob.test[ , h+2*NK]/
##D                             post$prob.test[ , h], 1, mean)
##D     RI.Th.B6[ , j] <- apply(post$prob.test[ , h+2*NK]/
##D                             post$prob.test[ , h+NK], 1, mean)
##D }
##D 
##D RI.B6.Pl.mu <- apply(RI.B6.Pl, 2, mean)
##D RI.B6.Pl.025 <- apply(RI.B6.Pl, 2, quantile, probs=0.025)
##D RI.B6.Pl.975 <- apply(RI.B6.Pl, 2, quantile, probs=0.975)
##D 
##D RI.Th.Pl.mu <- apply(RI.Th.Pl, 2, mean)
##D RI.Th.Pl.025 <- apply(RI.Th.Pl, 2, quantile, probs=0.025)
##D RI.Th.Pl.975 <- apply(RI.Th.Pl, 2, quantile, probs=0.975)
##D 
##D RI.Th.B6.mu <- apply(RI.Th.B6, 2, mean)
##D RI.Th.B6.025 <- apply(RI.Th.B6, 2, quantile, probs=0.025)
##D RI.Th.B6.975 <- apply(RI.Th.B6, 2, quantile, probs=0.975)
##D 
##D plot(post$times, RI.Th.Pl.mu, col='blue',
##D      log='y', main='Bladder cancer ex: Thiotepa vs. Placebo',
##D      type='l', ylim=c(0.1, 10), ylab='RI(t)', xlab='t (months)')
##D lines(post$times, RI.Th.Pl.025, col='red')
##D lines(post$times, RI.Th.Pl.975, col='red')
##D abline(h=1)
##D 
##D plot(post$times, RI.B6.Pl.mu, col='blue',
##D      log='y', main='Bladder cancer ex: Vitamin B6 vs. Placebo',
##D      type='l', ylim=c(0.1, 10), ylab='RI(t)', xlab='t (months)')
##D lines(post$times, RI.B6.Pl.025, col='red')
##D lines(post$times, RI.B6.Pl.975, col='red')
##D abline(h=1)
##D 
##D plot(post$times, RI.Th.B6.mu, col='blue',
##D      log='y', main='Bladder cancer ex: Thiotepa vs. Vitamin B6',
##D      type='l', ylim=c(0.1, 10), ylab='RI(t)', xlab='t (months)')
##D lines(post$times, RI.Th.B6.025, col='red')
##D lines(post$times, RI.Th.B6.975, col='red')
##D abline(h=1)
##D 
## End(Not run)
