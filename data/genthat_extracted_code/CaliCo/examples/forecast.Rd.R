...
### Name: forecast
### Title: Generates a forecast base on calibration run with 'calibrate'
### Aliases: forecast
...
##D ###### Prediction between 1 and 1.2
##D X.new <- cbind(seq(1,1.2,length.out=10),seq(1,1.2,length.out=10))
##D fr <- forecast(mdfit,X.new)
##D print(fr)
##D plot(fr,c(X[,1],X.new[,1]))
## End(Not run)
