...
##D 
##D crit.grid <- apply(design.grid, 1, AEI, model=model, new.noise.var=noise.var)
##D crit.grad <- t(apply(design.grid, 1, AEI.grad, model=model, new.noise.var=noise.var))
##D 
##D z.grid <- matrix(crit.grid, n.grid, n.grid)
##D contour(x.grid,y.grid, z.grid, 30)
##D title("AEI and its gradient")
...
##D {
##D  x <- design.grid[i,]
##D  arrows(x$Var1,x$Var2, x$Var1+crit.grad[i,1]*.6,x$Var2+crit.grad[i,2]*.6,
