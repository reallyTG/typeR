...
## Not run: 
##D #################### AMEs and two-way and three-way AMIEs ####################
##D ## Note: All pairs within thee-way interactions should show up in int2.formula (Strong Hierarchy).
##D fit3 <- CausalANOVA(formula=won ~ newRecordF + promise + coeth_voting + relevantdegree,
##D                     int2.formula = ~ newRecordF:promise + newRecordF:coeth_voting
##D                                        + promise:coeth_voting,
##D                     int3.formula = ~ newRecordF:promise:coeth_voting,
##D                     data=Carlson, pair.id=Carlson$contestresp,diff=TRUE,
##D                     cluster=Carlson$respcodeS, nway=3)
##D summary(fit3)
##D plot(fit3, type="AMIE", fac.name=c("newRecordF","promise", "coeth_voting"),space=25,adj.p=2.2)
## End(Not run)
