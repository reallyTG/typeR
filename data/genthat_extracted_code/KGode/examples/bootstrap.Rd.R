...
##D 
##D ##Use function 'rkg' to estimate the ode parameters. The standard gradient matching method is coded
##D ##in the the 'rkg' function. The parameter estimations are stored in the returned vector of 'rkg'.
##D ## Choose a kernel type for 'rkhs' interpolation. Two options are provided 'rbf' and 'mlp'.
##D ktype ='rbf'
##D rkgres = rkg(kkk,y_no,ktype)
##D ## show the results of ode parameter estimation using the standard gradient matching
##D kkk$ode_par
##D 
##D ## Perform bootstrap procedure to estimate the median absolute deviations of ode parameters
##D # here we get the resulting interpolation from gradient matching using 'rkg' for each ode state
##D bbb = rkgres$bbb
##D nst = length(bbb)
##D intp_data = list()
##D for( i in 1:nst) {
##D     intp_data[[i]] = bbb[[i]]$predictT(bbb[[i]]$t)$pred
##D }
##D K = 12 # the number of bootstrap replicates
##D mads = bootstrap(kkk, y_no, ktype, K, ode_par, intp_data)
##D 
##D ## show the results of ode parameter estimation and its uncertainty
##D ## using the standard gradient matching
##D ode_par
##D mads
##D 
##D ############# gradient matching + ODE regularisation
##D crtype='i'
##D lam=c(10,1,1e-1,1e-2,1e-4)
##D lamil1 = crossv(lam,kkk,bbb,crtype,y_no)
##D lambdai1=lamil1[[1]]
##D res = third(lambdai1,kkk,bbb,crtype)
##D oppar = res$oppar
##D 
##D ### do bootstrap here for gradient matching + ODE regularisation
##D ode_par = oppar
##D K = 12
##D intp_data = list()
##D for( i in 1:nst) {
##D     intp_data[[i]] = res$rk3$rk[[i]]$predictT(bbb[[i]]$t)$pred
##D }
##D mads = bootstrap(kkk, y_no, ktype, K, ode_par, intp_data)
##D ode_par
##D mads
##D 
##D ############# gradient matching + ODE regularisation + warping
##D ###### warp state
##D peod = c(6,5.3) #8#9.7     ## the guessing period
##D eps= 1          ## the standard deviation of period
##D fixlens=warpInitLen(peod,eps,rkgres)
##D kkkrkg = kkk$clone()
##D www = warpfun(kkkrkg,bbb,peod,eps,fixlens,y_no,kkkrkg$t)
##D 
##D ### do bootstrap here for gradient matching + ODE regularisation + warping
##D nst = length(bbb)
##D K = 12
##D ode_par = www$wkkk$ode_par
##D intp_data = list()
##D for( i in 1:nst) {
##D     intp_data[[i]] = www$bbbw[[i]]$predictT(www$wtime[i, ])$pred
##D }
##D mads = bootstrap(kkk, y_no, ktype, K, ode_par, intp_data,www)
##D ode_par
##D mads
##D 
## End(Not run)
