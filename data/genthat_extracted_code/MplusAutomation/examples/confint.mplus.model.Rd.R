library(MplusAutomation)


### Name: confint.mplus.model
### Title: Return confidence intervals for an mplus.model object
### Aliases: confint.mplus.model confint.mplusObject
### Keywords: interface

### ** Examples

## Not run: 
##D # simple example of a model using builtin data
##D # demonstrates use
##D test <- mplusObject(
##D   TITLE = "test the MplusAutomation Package;",
##D   MODEL = "
##D     mpg ON wt hp;
##D     wt WITH hp;",
##D   OUTPUT = "STANDARDIZED; CINTERVAL;",
##D   usevariables = c("mpg", "wt", "hp"),
##D   rdata = mtcars)
##D 
##D res <- mplusModeler(test, "mtcars.dat", modelout = "model1.inp", run = 1L)
##D 
##D # example of the confint method on an mplus.model object
##D # note that res$results holds the results of readModels()
##D confint(res$results)
##D confint(res$results, type = "std")
##D confint(res$results, type = "stdy")
##D confint(res$results, type = "stdyx", level = .99)
##D 
##D # there is also a method for mplusObject class
##D confint(res)
##D screenreg(res, cis = TRUE, single.row = TRUE)
##D 
##D # remove files
##D unlink("mtcars.dat")
##D unlink("model1.inp")
##D unlink("model1.out")
##D unlink("Mplus Run Models.log")
## End(Not run)



