...
##D ds_all_rows_all_fields <- redcap_read_oneshot(redcap_uri=uri, token=token)$data
##D 
##D #Return only records with IDs of 1 and 3
##D desired_records_v1 <- c(1, 3)
##D ds_some_rows_v1 <- redcap_read_oneshot(
##D    redcap_uri = uri,
##D    token      = token,
...
##D ds_some_fields_v1 <- redcap_read_oneshot(
##D    redcap_uri = uri,
##D    token      = token,
