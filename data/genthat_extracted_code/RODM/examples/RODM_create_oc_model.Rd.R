...
### Name: RODM_create_oc_model
### Title: Create an O-cluster model
### Aliases: RODM_create_oc_model
### Keywords: cluster
...
##D    num_clusters = 5)
##D 
##D oc2 <- RODM_apply_model(
##D    database = DB,                  # database ODBC channel identifier
##D    data_table_name = "ds",         # data frame containing the input dataset
##D    model_name = "OC_MODEL",
##D    supplemental_cols = c("X1","Y1"))
##D 
##D x1a <- oc2$model.apply.results[, "X1"]
##D y1a <- oc2$model.apply.results[, "Y1"]
##D clu <- oc2$model.apply.results[, "CLUSTER_ID"]
...
##D plot(x1a, y1a, pch=20, type = "n", xlab="X1", ylab="Y1", main="After OC clustering")
##D for (i in 1:n.rows) {
##D    points(x1a[i], y1a[i], col= color[i], pch=20)
##D }   
##D legend(5, -0.5, legend=c("Cluster 1", "Cluster 2", "Cluster 3"), pch = rep(20, 3), 
##D        col = color.map, pt.bg = color.map, cex = 0.8, pt.cex=1, bty="n")
##D 
##D oc        # look at the model details and cluster assignments
##D 
##D RODM_drop_model(DB, "OC_MODEL")   # Drop the database table
