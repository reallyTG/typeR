...
### Name: print.sienaMeta
### Title: Methods for processing sienaMeta objects
### Aliases: print.sienaMeta sienaMeta summary.sienaMeta plot.sienaMeta
...
##D # but it the Fisher combinations of p-values are meaningful.
##D # But using three groups shows the idea.
##D 
...
##D OneAlgorithm <- sienaAlgorithmCreate(projname = "SingleGroups")
...
##D # First determine the number of estimated effects:
##D (neff <- sum(sapply(meta, function(x){ifelse(is.list(x),!is.null(x$cjplus),FALSE)})))
##D Fishers <- t(sapply(1:neff,
##D         function(i){c(meta[[i]]$cjplus, meta[[i]]$cjminus,
##D                         meta[[i]]$cjplusp, meta[[i]]$cjminusp, 2*meta[[i]]$n1 )}))
##D Fishers <- as.data.frame(Fishers, row.names=names(meta)[1:neff])
##D names(Fishers) <- c('Fplus', 'Fminus', 'pplus', 'pminus', 'df')
##D Fishers
##D # For plotting:
##D plo <- plot(meta, layout = c(3,1))
##D plo
##D plo[3]
## End(Not run)
