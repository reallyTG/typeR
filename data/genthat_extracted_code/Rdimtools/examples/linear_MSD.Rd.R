...
##D out1 = do.msd(X, label, C=0.01)
##D out2 = do.msd(X, label, C=1)
##D out3 = do.msd(X, label, C=100)
##D 
##D ## visualize
##D par(mfrow=c(1,3))
##D plot(out1$Y[,1], out1$Y[,2], main="MSD::C=0.01")
##D plot(out2$Y[,1], out2$Y[,2], main="MSD::C=1")
##D plot(out3$Y[,1], out3$Y[,2], main="MSD::C=100")
## End(Not run)
