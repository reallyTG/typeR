...
##D 
##D ## perform MVP with different preprocessings
##D out1 = do.kmmc(X, label, t=0.1)
##D out2 = do.kmmc(X, label, t=1.0)
##D out3 = do.kmmc(X, label, t=10.0)
##D 
##D ## visualize
##D par(mfrow=c(1,3))
##D plot(out1$Y[,1], out1$Y[,2], main="bandwidth=0.1")
##D plot(out2$Y[,1], out2$Y[,2], main="bandwidth=1")
##D plot(out3$Y[,1], out3$Y[,2], main="bandwidth=10.0")
## End(Not run)
