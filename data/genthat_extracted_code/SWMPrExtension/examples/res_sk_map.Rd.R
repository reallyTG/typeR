library(SWMPrExtension)


### Name: res_sk_map
### Title: Local Reserve Map With Seasonal Kendall Results
### Aliases: res_sk_map

### ** Examples

## a compact reserve
### set plotting parameters
stations <-
sampling_stations[(sampling_stations$NERR.Site.ID == 'elk'
& sampling_stations$Status == 'Active'), ]$Station.Code
to_match <- c('wq')
stns <- stations[grep(paste(to_match, collapse = '|'), stations)]
shp_fl <- elk_spatial
bounding_elk <- c(-121.810978, 36.868218, -121.708667, 36.764050)
pos <- 'bottomleft'
sk_res <- c('inc', 'dec', 'dec', 'insig')

### plot
res_sk_map('elk', stations = stns, sk_result = sk_res,
bbox = bounding_elk, scale_pos = pos, shp = shp_fl)

## Not run: 
##D ## a multicomponent reserve (showing two different bounding boxes)
##D ### set plotting parameters
##D stations <-
##D sampling_stations[(sampling_stations$NERR.Site.ID == 'cbm'
##D & sampling_stations$Status == 'Active'), ]$Station.Code
##D to_match <- c('wq')
##D stns <- stations[grep(paste(to_match, collapse = '|'), stations)]
##D shp_fl <- cbm_spatial
##D bounding_cbm_1 <- c(-77.393, 39.741, -75.553, 38.277)
##D bounding_cbm_2 <- c(-76.862006, 38.811571, -76.596508, 38.642454)
##D pos <- 'bottomleft'
##D sk_res <- c('inc', 'dec', 'dec', 'insig')
##D 
##D ### plot
##D res_sk_map('cbm', stations = stns, sk_result = sk_res, bbox = bounding_cbm_1,
##D scale_pos = pos, shp = shp_fl)
##D 
##D res_sk_map('cbm', stations = stns, sk_result = sk_res, bbox = bounding_cbm_2,
##D scale_pos = pos, shp = shp_fl)
##D 
## End(Not run)




