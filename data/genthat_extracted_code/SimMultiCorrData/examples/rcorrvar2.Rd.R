...
###   Negative Binomial Variables: Correlation Method 2
### Aliases: rcorrvar2
### Keywords: Binomial, Fleishman, Headrick, Negative Poisson, continuous,
###   method2 ordinal, simulation,
...
##D valid2 <- valid_corr2(k_cat = 1, k_cont = 2, k_pois = 1, k_nb = 1,
...
##D                       lam = lam, pois_eps = pois_eps, size = size,
##D                       prob = prob, nb_eps = nb_eps, rho = Rey,
##D                       seed = seed)
##D 
##D # Simulate variables without error loop
##D Sim2 <- rcorrvar2(n = n, k_cat = 1, k_cont = 2, k_pois = 1, k_nb = 1,
##D                   method = "Polynomial", means = Stcum[, 1],
##D                   vars = Stcum[, 2]^2, skews = Stcum[, 3],
##D                   skurts = Stcum[, 4], fifths = Stcum[, 5],
##D                   sixths = Stcum[, 6], Six = Six, marginal = marginal,
...
##D Sim2$maxerr
##D 
##D Sim2_error = round(Sim2$correlations - Rey, 6)
##D 
##D # interquartile-range of correlation errors
##D quantile(as.numeric(Sim2_error), 0.25)
##D quantile(as.numeric(Sim2_error), 0.75)
##D 
##D # Simulate variables with error loop
##D Sim2_EL <- rcorrvar2(n = n, k_cat = 1, k_cont = 2, k_pois = 1, k_nb = 1,
##D                      method = "Polynomial", means = Stcum[, 1],
##D                      vars = Stcum[, 2]^2, skews = Stcum[, 3],
##D                      skurts = Stcum[, 4], fifths = Stcum[, 5],
##D                      sixths = Stcum[, 6], Six = Six, marginal = marginal,
...
##D Sim2_EL$maxerr
##D 
##D EL_error = round(Sim2_EL$correlations - Rey, 6)
##D 
##D # interquartile-range of correlation errors
...
##D # Look at results
##D # Ordinal variables
##D Sim2_EL$summary_ordinal
##D 
##D # Continuous variables
##D round(Sim2_EL$constants, 6)
##D round(Sim2_EL$summary_continuous, 6)
##D round(Sim2_EL$summary_targetcont, 6)
##D Sim2_EL$valid.pdf
##D 
##D # Count variables
##D Sim2_EL$summary_Poisson
##D Sim2_EL$summary_Neg_Bin
##D 
##D # Generate Plots
##D 
##D # Logistic (1st continuous variable)
##D # 1) Simulated Data CDF (find cumulative probability up to y = 0.5)
##D plot_sim_cdf(Sim2_EL$continuous_variables[, 1], calc_cprob = TRUE,
##D              delta = 0.5)
##D 
##D # 2) Simulated Data and Target Distribution PDFs
##D plot_sim_pdf_theory(Sim2_EL$continuous_variables[, 1], Dist = "Logistic",
##D                     params = c(0, 1))
##D 
##D # 3) Simulated Data and Target Distribution
##D plot_sim_theory(Sim2_EL$continuous_variables[, 1], Dist = "Logistic",
##D                 params = c(0, 1))
##D 
## End(Not run)
