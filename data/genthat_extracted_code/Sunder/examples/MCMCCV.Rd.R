...
## Not run: 
##D data(toydata,package='Sunder')
##D      
##D      #### Computing options
##D      nit <- 10^2
##D      run  <- c(1,1,1)
##D      thinning  <- 1 # max(nit/10^3,1);
##D      ud   <- c(0,1,1,0,0) 
##D      theta.init <- c(1,2,1,1,0.01)
##D      n.validation.set  <- dim(gen)[1]*dim(gen)[2]/10 
...
##D      
##D      #### Call Sunder ####
##D      output <- MCMCCV(gen,D_G,D_E,
##D                                 nit,thinning,
##D                                 theta.max,
##D                                 theta.init,
##D                                 run,ud,n.validation.set)
##D      
...
##D 
##D print(mod.lik)
##D print(paste('The model achieving the highest likelihood on the validation set is:',
##D             names(mod.lik)[order(mod.lik,decreasing=TRUE)[1]]))
##D theta.GE <- apply(output$theta[,,1],1,mean)
##D print('Posterior mean theta under model G+E:')
##D print(theta.GE)
##D 
##D theta.G <- apply(output$theta[,,2],1,mean)
##D theta.G[3] <- NA
##D print('Posterior mean theta under model G:')
##D print(theta.G)
##D 
##D theta.E <- apply(output$theta[,,3],1,mean)
##D theta.E[2] <- NA
##D print('Posterior mean theta under model E:')
##D print(theta.E)
##D 
## End(Not run)
