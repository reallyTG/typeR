...
color.map<- function(x,dem,y=NULL){
	...
	dem.num<- cut(dem ,breaks=ceiling(quantile(dem,na.rm=TRUE)),dig.lab = 6)
...
colors.use<-color.map(alaska.cdp,alaska.cdp$pop2000)
plot(alaska.cdp,col=colors.use$colors,ylim=c(51.78495, 71.33953),xlim=c(-176.81043, -130.0427))
#text(coordinates(alaska.cdp),alaska.cdp$name,cex=.3)
...
##D 
##D 
##D plotvar <- alaska.cdp$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(alaska.cdp, col=colcode, border="transparent",ylim=c(51.78495, 71.33953),xlim=c(-176.81043, -130.0427))
##D 	#transparent
