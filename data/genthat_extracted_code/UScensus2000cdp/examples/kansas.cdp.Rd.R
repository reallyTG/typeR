...
colors.use<-color.map(kansas.cdp,kansas.cdp$pop2000)
plot(kansas.cdp,col=colors.use$colors)
#text(coordinates(kansas.cdp),kansas.cdp$name,cex=.3)
...
##D data(kansas.cdp)
##D 
##D map('state',region='kansas')
##D plotvar <- kansas.cdp$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(kansas.cdp, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Designated Places\n of Kansas, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "kansas",cex=.7,add=TRUE)
##D map('county','kansas',add=TRUE)
