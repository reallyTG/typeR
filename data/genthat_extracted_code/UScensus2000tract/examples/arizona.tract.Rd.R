...
colors.use<-color.map(arizona.tract,arizona.tract$pop2000)
plot(arizona.tract,col=colors.use$colors)
#text(coordinates(arizona.tract),arizona.tract@data$name,cex=.3)
title(main="Census Tracts \n of Arizona, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(arizona.tract)
##D 
##D map('state',region='arizona')
##D plotvar <- arizona.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(arizona.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Arizona, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "arizona",cex=.7,add=TRUE)
##D map('county','arizona',add=TRUE)
