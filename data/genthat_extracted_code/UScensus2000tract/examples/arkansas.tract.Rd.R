...
### Name: arkansas.tract
### Title: arkansas.tract
### Aliases: arkansas.tract
### Keywords: datasets
...
colors.use<-color.map(arkansas.tract,arkansas.tract$pop2000)
plot(arkansas.tract,col=colors.use$colors)
#text(coordinates(arkansas.tract),arkansas.tract@data$name,cex=.3)
title(main="Census Tracts \n of Arkansas, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(arkansas.tract)
##D 
##D map('state',region='arkansas')
##D plotvar <- arkansas.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(arkansas.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Arkansas, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "arkansas",cex=.7,add=TRUE)
##D map('county','arkansas',add=TRUE)
