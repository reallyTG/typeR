...
### Name: california.tract
### Title: california.tract
### Aliases: california.tract
### Keywords: datasets
...
colors.use<-color.map(california.tract,california.tract$pop2000)
plot(california.tract,col=colors.use$colors)
#text(coordinates(california.tract),california.tract@data$name,cex=.3)
...
##D ####This example requires the following additional libraries
##D 
##D data(california.tract)
##D 
##D map('state',region='california')
##D plotvar <- california.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(california.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of California, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "california",cex=.7,add=TRUE)
##D map('county','california',add=TRUE)
