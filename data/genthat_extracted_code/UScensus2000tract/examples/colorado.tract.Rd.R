...
### Name: colorado.tract
### Title: colorado.tract
### Aliases: colorado.tract
### Keywords: datasets
...
colors.use<-color.map(colorado.tract,colorado.tract$pop2000)
plot(colorado.tract,col=colors.use$colors)
#text(coordinates(colorado.tract),colorado.tract@data$name,cex=.3)
title(main="Census Tracts \n of Colorado, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(colorado.tract)
##D 
##D map('state',region='colorado')
##D plotvar <- colorado.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(colorado.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Colorado, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "colorado",cex=.7,add=TRUE)
##D map('county','colorado',add=TRUE)
