...
### Name: delaware.tract
### Title: delaware.tract
### Aliases: delaware.tract
### Keywords: datasets
...
colors.use<-color.map(delaware.tract,delaware.tract$pop2000)
plot(delaware.tract,col=colors.use$colors)
#text(coordinates(delaware.tract),delaware.tract@data$name,cex=.3)
...
##D ####This example requires the following additional libraries
##D 
##D data(delaware.tract)
##D 
##D map('state',region='delaware')
##D plotvar <- delaware.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(delaware.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Delaware, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "delaware",cex=.7,add=TRUE)
##D map('county','delaware',add=TRUE)
