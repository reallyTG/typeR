...
colors.use<-color.map(florida.tract,florida.tract$pop2000)
plot(florida.tract,col=colors.use$colors)
#text(coordinates(florida.tract),florida.tract@data$name,cex=.3)
title(main="Census Tracts \n of Florida, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(florida.tract)
##D 
##D map('state',region='florida')
##D plotvar <- florida.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(florida.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Florida, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "florida",cex=.7,add=TRUE)
##D map('county','florida',add=TRUE)
