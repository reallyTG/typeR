...
colors.use<-color.map(georgia.tract,georgia.tract$pop2000)
plot(georgia.tract,col=colors.use$colors)
#text(coordinates(georgia.tract),georgia.tract@data$name,cex=.3)
title(main="Census Tracts \n of Georgia, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(georgia.tract)
##D 
##D map('state',region='georgia')
##D plotvar <- georgia.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(georgia.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Georgia, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "georgia",cex=.7,add=TRUE)
##D map('county','georgia',add=TRUE)
