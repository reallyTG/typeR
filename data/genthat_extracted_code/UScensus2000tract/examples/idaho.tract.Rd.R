...
colors.use<-color.map(idaho.tract,idaho.tract$pop2000)
plot(idaho.tract,col=colors.use$colors)
#text(coordinates(idaho.tract),idaho.tract@data$name,cex=.3)
title(main="Census Tracts \n of Idaho, 2000", sub="Quantiles (equal frequency)")
...
##D data(idaho.tract)
##D 
##D map('state',region='idaho')
##D plotvar <- idaho.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(idaho.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Idaho, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "idaho",cex=.7,add=TRUE)
##D map('county','idaho',add=TRUE)
