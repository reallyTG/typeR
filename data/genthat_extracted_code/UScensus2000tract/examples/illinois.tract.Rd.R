...
### Name: illinois.tract
### Title: illinois.tract
### Aliases: illinois.tract
### Keywords: datasets
...
colors.use<-color.map(illinois.tract,illinois.tract$pop2000)
plot(illinois.tract,col=colors.use$colors)
#text(coordinates(illinois.tract),illinois.tract@data$name,cex=.3)
title(main="Census Tracts \n of Illinois, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(illinois.tract)
##D 
##D map('state',region='illinois')
##D plotvar <- illinois.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(illinois.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Illinois, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "illinois",cex=.7,add=TRUE)
##D map('county','illinois',add=TRUE)
