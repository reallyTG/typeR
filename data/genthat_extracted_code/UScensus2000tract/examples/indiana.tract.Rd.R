...
colors.use<-color.map(indiana.tract,indiana.tract$pop2000)
plot(indiana.tract,col=colors.use$colors)
#text(coordinates(indiana.tract),indiana.tract@data$name,cex=.3)
title(main="Census Tracts \n of Indiana, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(indiana.tract)
##D 
##D map('state',region='indiana')
##D plotvar <- indiana.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(indiana.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Indiana, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "indiana",cex=.7,add=TRUE)
##D map('county','indiana',add=TRUE)
