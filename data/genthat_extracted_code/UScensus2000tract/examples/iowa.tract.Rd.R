...
colors.use<-color.map(iowa.tract,iowa.tract$pop2000)
plot(iowa.tract,col=colors.use$colors)
#text(coordinates(iowa.tract),iowa.tract@data$name,cex=.3)
title(main="Census Tracts \n of Iowa, 2000", sub="Quantiles (equal frequency)")
...
##D data(iowa.tract)
##D 
##D map('state',region='iowa')
##D plotvar <- iowa.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(iowa.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Iowa, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "iowa",cex=.7,add=TRUE)
##D map('county','iowa',add=TRUE)
