...
### Name: kentucky.tract
### Title: kentucky.tract
### Aliases: kentucky.tract
### Keywords: datasets
...
colors.use<-color.map(kentucky.tract,kentucky.tract$pop2000)
plot(kentucky.tract,col=colors.use$colors)
#text(coordinates(kentucky.tract),kentucky.tract@data$name,cex=.3)
title(main="Census Tracts \n of Kentucky, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(kentucky.tract)
##D 
##D map('state',region='kentucky')
##D plotvar <- kentucky.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(kentucky.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Kentucky, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "kentucky",cex=.7,add=TRUE)
##D map('county','kentucky',add=TRUE)
