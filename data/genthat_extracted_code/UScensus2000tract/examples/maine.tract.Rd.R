...
colors.use<-color.map(maine.tract,maine.tract$pop2000)
plot(maine.tract,col=colors.use$colors)
#text(coordinates(maine.tract),maine.tract@data$name,cex=.3)
title(main="Census Tracts \n of Maine, 2000", sub="Quantiles (equal frequency)")
...
##D data(maine.tract)
##D 
##D map('state',region='maine')
##D plotvar <- maine.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(maine.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Maine, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "maine",cex=.7,add=TRUE)
##D map('county','maine',add=TRUE)
