...
### Name: maryland.tract
### Title: maryland.tract
### Aliases: maryland.tract
### Keywords: datasets
...
colors.use<-color.map(maryland.tract,maryland.tract$pop2000)
plot(maryland.tract,col=colors.use$colors)
#text(coordinates(maryland.tract),maryland.tract@data$name,cex=.3)
title(main="Census Tracts \n of Maryland, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(maryland.tract)
##D 
##D map('state',region='maryland')
##D plotvar <- maryland.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(maryland.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Maryland, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "maryland",cex=.7,add=TRUE)
##D map('county','maryland',add=TRUE)
