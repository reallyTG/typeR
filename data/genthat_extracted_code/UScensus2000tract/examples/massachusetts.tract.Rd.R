...
### Name: massachusetts.tract
### Title: massachusetts.tract
### Aliases: massachusetts.tract
### Keywords: datasets

### ** Examples

data(massachusetts.tract)
...
colors.use<-color.map(massachusetts.tract,massachusetts.tract$pop2000)
plot(massachusetts.tract,col=colors.use$colors)
#text(coordinates(massachusetts.tract),massachusetts.tract@data$name,cex=.3)
title(main="Census Tracts \n of Massachusetts, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(massachusetts.tract)
##D 
##D map('state',region='massachusetts')
##D plotvar <- massachusetts.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(massachusetts.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Massachusetts, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "massachusetts",cex=.7,add=TRUE)
##D map('county','massachusetts',add=TRUE)
