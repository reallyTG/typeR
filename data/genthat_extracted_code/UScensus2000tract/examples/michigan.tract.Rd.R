...
### Name: michigan.tract
### Title: michigan.tract
### Aliases: michigan.tract
### Keywords: datasets
...
colors.use<-color.map(michigan.tract,michigan.tract$pop2000)
plot(michigan.tract,col=colors.use$colors)
#text(coordinates(michigan.tract),michigan.tract@data$name,cex=.3)
title(main="Census Tracts \n of Michigan, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(michigan.tract)
##D 
##D map('state',region='michigan')
##D plotvar <- michigan.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(michigan.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Michigan, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "michigan",cex=.7,add=TRUE)
##D map('county','michigan',add=TRUE)
