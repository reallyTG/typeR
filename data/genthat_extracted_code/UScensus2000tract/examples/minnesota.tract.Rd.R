...
### Name: minnesota.tract
### Title: minnesota.tract
### Aliases: minnesota.tract
### Keywords: datasets
...
colors.use<-color.map(minnesota.tract,minnesota.tract$pop2000)
plot(minnesota.tract,col=colors.use$colors)
#text(coordinates(minnesota.tract),minnesota.tract@data$name,cex=.3)
title(main="Census Tracts \n of Minnesota, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(minnesota.tract)
##D 
##D map('state',region='minnesota')
##D plotvar <- minnesota.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(minnesota.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Minnesota, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "minnesota",cex=.7,add=TRUE)
##D map('county','minnesota',add=TRUE)
