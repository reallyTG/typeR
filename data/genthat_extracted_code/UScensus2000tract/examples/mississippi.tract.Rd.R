...
### Name: mississippi.tract
### Title: mississippi.tract
### Aliases: mississippi.tract
### Keywords: datasets

### ** Examples

data(mississippi.tract)
...
colors.use<-color.map(mississippi.tract,mississippi.tract$pop2000)
plot(mississippi.tract,col=colors.use$colors)
#text(coordinates(mississippi.tract),mississippi.tract@data$name,cex=.3)
title(main="Census Tracts \n of Mississippi, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(mississippi.tract)
##D 
##D map('state',region='mississippi')
##D plotvar <- mississippi.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(mississippi.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Mississippi, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "mississippi",cex=.7,add=TRUE)
##D map('county','mississippi',add=TRUE)
