...
### Name: missouri.tract
### Title: missouri.tract
### Aliases: missouri.tract
### Keywords: datasets
...
colors.use<-color.map(missouri.tract,missouri.tract$pop2000)
plot(missouri.tract,col=colors.use$colors)
#text(coordinates(missouri.tract),missouri.tract@data$name,cex=.3)
title(main="Census Tracts \n of Missouri, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(missouri.tract)
##D 
##D map('state',region='missouri')
##D plotvar <- missouri.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(missouri.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Missouri, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "missouri",cex=.7,add=TRUE)
##D map('county','missouri',add=TRUE)
