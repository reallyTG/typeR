...
colors.use<-color.map(montana.tract,montana.tract$pop2000)
plot(montana.tract,col=colors.use$colors)
#text(coordinates(montana.tract),montana.tract@data$name,cex=.3)
title(main="Census Tracts \n of Montana, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(montana.tract)
##D 
##D map('state',region='montana')
##D plotvar <- montana.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(montana.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Montana, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "montana",cex=.7,add=TRUE)
##D map('county','montana',add=TRUE)
