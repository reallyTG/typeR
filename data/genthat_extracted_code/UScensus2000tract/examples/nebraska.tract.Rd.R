...
### Name: nebraska.tract
### Title: nebraska.tract
### Aliases: nebraska.tract
### Keywords: datasets
...
colors.use<-color.map(nebraska.tract,nebraska.tract$pop2000)
plot(nebraska.tract,col=colors.use$colors)
#text(coordinates(nebraska.tract),nebraska.tract@data$name,cex=.3)
title(main="Census Tracts \n of Nebraska, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(nebraska.tract)
##D 
##D map('state',region='nebraska')
##D plotvar <- nebraska.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(nebraska.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Nebraska, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "nebraska",cex=.7,add=TRUE)
##D map('county','nebraska',add=TRUE)
