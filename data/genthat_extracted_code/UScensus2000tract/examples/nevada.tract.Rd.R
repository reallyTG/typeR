...
colors.use<-color.map(nevada.tract,nevada.tract$pop2000)
plot(nevada.tract,col=colors.use$colors)
#text(coordinates(nevada.tract),nevada.tract@data$name,cex=.3)
title(main="Census Tracts \n of Nevada, 2000", sub="Quantiles (equal frequency)")
...
##D data(nevada.tract)
##D 
##D map('state',region='nevada')
##D plotvar <- nevada.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(nevada.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Nevada, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "nevada",cex=.7,add=TRUE)
##D map('county','nevada',add=TRUE)
