...
### Name: new_hampshire.tract
### Title: new_hampshire.tract
### Aliases: new_hampshire.tract
### Keywords: datasets

### ** Examples

data(new_hampshire.tract)
...
colors.use<-color.map(new_hampshire.tract,new_hampshire.tract$pop2000)
plot(new_hampshire.tract,col=colors.use$colors)
#text(coordinates(new_hampshire.tract),new_hampshire.tract@data$name,cex=.3)
title(main="Census Tracts \n of New_hampshire, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(new_hampshire.tract)
##D 
##D map('state',region='new_hampshire')
##D plotvar <- new_hampshire.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(new_hampshire.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of New_hampshire, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "new_hampshire",cex=.7,add=TRUE)
##D map('county','new_hampshire',add=TRUE)
