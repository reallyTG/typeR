...
### Name: new_jersey.tract
### Title: new_jersey.tract
### Aliases: new_jersey.tract
### Keywords: datasets

### ** Examples

data(new_jersey.tract)
...
colors.use<-color.map(new_jersey.tract,new_jersey.tract$pop2000)
plot(new_jersey.tract,col=colors.use$colors)
#text(coordinates(new_jersey.tract),new_jersey.tract@data$name,cex=.3)
...
##D ####This example requires the following additional libraries
##D 
##D data(new_jersey.tract)
##D 
##D map('state',region='new_jersey')
##D plotvar <- new_jersey.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(new_jersey.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of New_jersey, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "new_jersey",cex=.7,add=TRUE)
##D map('county','new_jersey',add=TRUE)
