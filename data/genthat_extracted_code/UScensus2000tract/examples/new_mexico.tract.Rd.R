...
### Name: new_mexico.tract
### Title: new_mexico.tract
### Aliases: new_mexico.tract
### Keywords: datasets

### ** Examples

data(new_mexico.tract)
...
colors.use<-color.map(new_mexico.tract,new_mexico.tract$pop2000)
plot(new_mexico.tract,col=colors.use$colors)
#text(coordinates(new_mexico.tract),new_mexico.tract@data$name,cex=.3)
title(main="Census Tracts \n of New_mexico, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(new_mexico.tract)
##D 
##D map('state',region='new_mexico')
##D plotvar <- new_mexico.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(new_mexico.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of New_mexico, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "new_mexico",cex=.7,add=TRUE)
##D map('county','new_mexico',add=TRUE)
