...
### Name: new_york.tract
### Title: new_york.tract
### Aliases: new_york.tract
### Keywords: datasets
...
colors.use<-color.map(new_york.tract,new_york.tract$pop2000)
plot(new_york.tract,col=colors.use$colors)
#text(coordinates(new_york.tract),new_york.tract@data$name,cex=.3)
title(main="Census Tracts \n of New_york, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(new_york.tract)
##D 
##D map('state',region='new_york')
##D plotvar <- new_york.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(new_york.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of New_york, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "new_york",cex=.7,add=TRUE)
##D map('county','new_york',add=TRUE)
