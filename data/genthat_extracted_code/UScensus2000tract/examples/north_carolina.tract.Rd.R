...
### Name: north_carolina.tract
### Title: north_carolina.tract
### Aliases: north_carolina.tract
### Keywords: datasets

### ** Examples

data(north_carolina.tract)
...
colors.use<-color.map(north_carolina.tract,north_carolina.tract$pop2000)
plot(north_carolina.tract,col=colors.use$colors)
#text(coordinates(north_carolina.tract),north_carolina.tract@data$name,cex=.3)
title(main="Census Tracts \n of North_carolina, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(north_carolina.tract)
##D 
##D map('state',region='north_carolina')
##D plotvar <- north_carolina.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(north_carolina.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of North_carolina, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "north_carolina",cex=.7,add=TRUE)
##D map('county','north_carolina',add=TRUE)
