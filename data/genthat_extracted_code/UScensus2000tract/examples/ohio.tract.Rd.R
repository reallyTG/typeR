...
colors.use<-color.map(ohio.tract,ohio.tract$pop2000)
plot(ohio.tract,col=colors.use$colors)
#text(coordinates(ohio.tract),ohio.tract@data$name,cex=.3)
title(main="Census Tracts \n of Ohio, 2000", sub="Quantiles (equal frequency)")
...
##D data(ohio.tract)
##D 
##D map('state',region='ohio')
##D plotvar <- ohio.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(ohio.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Ohio, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "ohio",cex=.7,add=TRUE)
##D map('county','ohio',add=TRUE)
