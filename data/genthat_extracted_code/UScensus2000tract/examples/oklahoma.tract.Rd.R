...
### Name: oklahoma.tract
### Title: oklahoma.tract
### Aliases: oklahoma.tract
### Keywords: datasets
...
colors.use<-color.map(oklahoma.tract,oklahoma.tract$pop2000)
plot(oklahoma.tract,col=colors.use$colors)
#text(coordinates(oklahoma.tract),oklahoma.tract@data$name,cex=.3)
title(main="Census Tracts \n of Oklahoma, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(oklahoma.tract)
##D 
##D map('state',region='oklahoma')
##D plotvar <- oklahoma.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(oklahoma.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Oklahoma, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "oklahoma",cex=.7,add=TRUE)
##D map('county','oklahoma',add=TRUE)
