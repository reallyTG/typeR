...
### Name: rhode_island.tract
### Title: rhode_island.tract
### Aliases: rhode_island.tract
### Keywords: datasets

### ** Examples

data(rhode_island.tract)
...
colors.use<-color.map(rhode_island.tract,rhode_island.tract$pop2000)
plot(rhode_island.tract,col=colors.use$colors)
#text(coordinates(rhode_island.tract),rhode_island.tract@data$name,cex=.3)
title(main="Census Tracts \n of Rhode_island, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(rhode_island.tract)
##D 
##D map('state',region='rhode_island')
##D plotvar <- rhode_island.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(rhode_island.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Rhode_island, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "rhode_island",cex=.7,add=TRUE)
##D map('county','rhode_island',add=TRUE)
