...
### Name: south_dakota.tract
### Title: south_dakota.tract
### Aliases: south_dakota.tract
### Keywords: datasets

### ** Examples

data(south_dakota.tract)
...
colors.use<-color.map(south_dakota.tract,south_dakota.tract$pop2000)
plot(south_dakota.tract,col=colors.use$colors)
#text(coordinates(south_dakota.tract),south_dakota.tract@data$name,cex=.3)
title(main="Census Tracts \n of South_dakota, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(south_dakota.tract)
##D 
##D map('state',region='south_dakota')
##D plotvar <- south_dakota.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(south_dakota.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of South_dakota, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "south_dakota",cex=.7,add=TRUE)
##D map('county','south_dakota',add=TRUE)
