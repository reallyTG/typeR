...
colors.use<-color.map(texas.tract,texas.tract$pop2000)
plot(texas.tract,col=colors.use$colors)
#text(coordinates(texas.tract),texas.tract@data$name,cex=.3)
title(main="Census Tracts \n of Texas, 2000", sub="Quantiles (equal frequency)")
...
##D data(texas.tract)
##D 
##D map('state',region='texas')
##D plotvar <- texas.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(texas.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Texas, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "texas",cex=.7,add=TRUE)
##D map('county','texas',add=TRUE)
