...
colors.use<-color.map(utah.tract,utah.tract$pop2000)
plot(utah.tract,col=colors.use$colors)
#text(coordinates(utah.tract),utah.tract@data$name,cex=.3)
title(main="Census Tracts \n of Utah, 2000", sub="Quantiles (equal frequency)")
...
##D data(utah.tract)
##D 
##D map('state',region='utah')
##D plotvar <- utah.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(utah.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Utah, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "utah",cex=.7,add=TRUE)
##D map('county','utah',add=TRUE)
