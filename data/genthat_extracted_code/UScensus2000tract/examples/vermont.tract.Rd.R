...
colors.use<-color.map(vermont.tract,vermont.tract$pop2000)
plot(vermont.tract,col=colors.use$colors)
#text(coordinates(vermont.tract),vermont.tract@data$name,cex=.3)
title(main="Census Tracts \n of Vermont, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(vermont.tract)
##D 
##D map('state',region='vermont')
##D plotvar <- vermont.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(vermont.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Vermont, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "vermont",cex=.7,add=TRUE)
##D map('county','vermont',add=TRUE)
