...
### Name: virginia.tract
### Title: virginia.tract
### Aliases: virginia.tract
### Keywords: datasets
...
colors.use<-color.map(virginia.tract,virginia.tract$pop2000)
plot(virginia.tract,col=colors.use$colors)
#text(coordinates(virginia.tract),virginia.tract@data$name,cex=.3)
title(main="Census Tracts \n of Virginia, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(virginia.tract)
##D 
##D map('state',region='virginia')
##D plotvar <- virginia.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(virginia.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Virginia, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "virginia",cex=.7,add=TRUE)
##D map('county','virginia',add=TRUE)
