...
### Name: washington.tract
### Title: washington.tract
### Aliases: washington.tract
### Keywords: datasets
...
colors.use<-color.map(washington.tract,washington.tract$pop2000)
plot(washington.tract,col=colors.use$colors)
#text(coordinates(washington.tract),washington.tract@data$name,cex=.3)
title(main="Census Tracts \n of Washington, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(washington.tract)
##D 
##D map('state',region='washington')
##D plotvar <- washington.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(washington.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Washington, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "washington",cex=.7,add=TRUE)
##D map('county','washington',add=TRUE)
