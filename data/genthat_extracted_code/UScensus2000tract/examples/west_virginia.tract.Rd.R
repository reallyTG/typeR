...
### Name: west_virginia.tract
### Title: west_virginia.tract
### Aliases: west_virginia.tract
### Keywords: datasets

### ** Examples

data(west_virginia.tract)
...
colors.use<-color.map(west_virginia.tract,west_virginia.tract$pop2000)
plot(west_virginia.tract,col=colors.use$colors)
#text(coordinates(west_virginia.tract),west_virginia.tract@data$name,cex=.3)
title(main="Census Tracts \n of West_virginia, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(west_virginia.tract)
##D 
##D map('state',region='west_virginia')
##D plotvar <- west_virginia.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(west_virginia.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of West_virginia, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "west_virginia",cex=.7,add=TRUE)
##D map('county','west_virginia',add=TRUE)
