...
### Name: wisconsin.tract
### Title: wisconsin.tract
### Aliases: wisconsin.tract
### Keywords: datasets

### ** Examples

data(wisconsin.tract)
...
colors.use<-color.map(wisconsin.tract,wisconsin.tract$pop2000)
plot(wisconsin.tract,col=colors.use$colors)
#text(coordinates(wisconsin.tract),wisconsin.tract@data$name,cex=.3)
title(main="Census Tracts \n of Wisconsin, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(wisconsin.tract)
##D 
##D map('state',region='wisconsin')
##D plotvar <- wisconsin.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(wisconsin.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Wisconsin, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "wisconsin",cex=.7,add=TRUE)
##D map('county','wisconsin',add=TRUE)
