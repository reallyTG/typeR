...
colors.use<-color.map(wyoming.tract,wyoming.tract$pop2000)
plot(wyoming.tract,col=colors.use$colors)
#text(coordinates(wyoming.tract),wyoming.tract@data$name,cex=.3)
title(main="Census Tracts \n of Wyoming, 2000", sub="Quantiles (equal frequency)")
...
##D ####This example requires the following additional libraries
##D 
##D data(wyoming.tract)
##D 
##D map('state',region='wyoming')
##D plotvar <- wyoming.tract$pop2000
##D 	nclr <- 4
##D 	#BuPu
##D 	plotclr <- brewer.pal(nclr,"BuPu")
##D 	class <- classIntervals(plotvar, nclr, style="quantile")
##D 	colcode <- findColours(class, plotclr)
##D 	plot(wyoming.tract, col=colcode, border="transparent",add=TRUE)
##D 	#transparent
##D title(main="Census Tracts \n of Wyoming, 2000", sub="Quantiles (equal frequency)")
##D map.text("county", "wyoming",cex=.7,add=TRUE)
##D map('county','wyoming',add=TRUE)
