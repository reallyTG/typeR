...
##D Mu1<-8*matrix(runif(Q*Q),Q,Q)
##D Mu2<-8*matrix(runif(Q*Q),Q,Q)
##D M1<-matrix(rnorm(n*n,sd=5),n,n)+Z%*%Mu1%*%t(Z) ## adjacency
##D M2<-matrix(rnorm(n*n,sd=10),n,n)+Z%*%Mu2%*%t(Z) ## adjacency
##D 
##D ## estimation
##D my_model <- BM_gaussian_multivariate_independent("SBM",list(M1,M2) )
##D my_model$estimate()
...
##D 
##D ## estimation
##D my_model <- BM_gaussian_multivariate_independent("SBM_sym",list(M1,M2) )
##D my_model$estimate()
##D which.max(my_model$ICL)
##D 
##D ##
...
##D Mu1<-8*matrix(runif(Q[1]*Q[2]),Q[1],Q[2])
##D Mu2<-8*matrix(runif(Q[1]*Q[2]),Q[1],Q[2])
##D M1<-matrix(rnorm(n[1]*n[2],sd=5),n[1],n[2])+Z1%*%Mu1%*%t(Z2) ## adjacency
##D M2<-matrix(rnorm(n[1]*n[2],sd=10),n[1],n[2])+Z1%*%Mu2%*%t(Z2) ## adjacency
##D 
##D ## estimation
##D my_model <- BM_gaussian_multivariate_independent("LBM",list(M1,M2) )
##D my_model$estimate()
##D which.max(my_model$ICL)
## End(Not run)
