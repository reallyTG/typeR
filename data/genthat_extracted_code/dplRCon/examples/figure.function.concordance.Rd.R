library(dplRCon)


### Name: figure.function.concordance
### Title: Figure: Plots the Concordances
### Aliases: figure.function.concordance

### ** Examples

## Not run: 
##D #Subset "near-pith" is the material within 0 -20cm from the estimated pith
##D spline200.sub0.20.n   <- TruncSeriesPithoffset( ring.raw, ring.stand, dbh.po.nc, c(1,200))
##D spline200.sub20.2000.n  <- TruncSeriesPithoffset( ring.raw, ring.stand, dbh.po.nc, c(200,200000))
##D boot.0.20   <-  series.bootstrap( spline200.sub0.20.n$sub.series.stand, stat, 999,
##D     names.stat, aver.by.tree = FALSE)
##D boot.20.2000   <- series.bootstrap(spline200.sub20.2000.n$sub.series.stand, stat, 999,
##D    names.stat, aver.by.tree = FALSE)
##D 
##D overall.precision.HUP <- overall.concordance.period(spline200.sub20.2000.n$sub.series.stand ,
##D    spline200.sub0.20.n$sub.series.stand,
##D    boot.20.2000$boot.series.mean,  boot.0.20$boot.series.mean ,1 , concordance.indices,
##D    c(1880,1999), trim.alpha=0.005, concordance.beta=0.5)
##D 
##D figure.function.concordance (overall.precision.HUP, x.lim=c(1880,2000))
## End(Not run)



