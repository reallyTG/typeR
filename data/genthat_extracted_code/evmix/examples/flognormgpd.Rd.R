library(evmix)


### Name: flognormgpd
### Title: MLE Fitting of log-normal Bulk and GPD Tail Extreme Value
###   Mixture Model
### Aliases: flognormgpd llognormgpd nllognormgpd proflulognormgpd
###   nlulognormgpd llognormgpd flognormgpd nllognormgpd proflulognormgpd
###   nlulognormgpd nllognormgpd flognormgpd llognormgpd proflulognormgpd
###   nlulognormgpd proflulognormgpd flognormgpd llognormgpd nllognormgpd
###   nlulognormgpd nlulognormgpd flognormgpd llognormgpd nllognormgpd
###   proflulognormgpd

### ** Examples

## Not run: 
##D set.seed(1)
##D par(mfrow = c(2, 1))
##D 
##D x = rlnorm(1000)
##D xx = seq(-0.1, 10, 0.01)
##D y = dlnorm(xx)
##D 
##D # Bulk model based tail fraction
##D fit = flognormgpd(x)
##D hist(x, breaks = 100, freq = FALSE, xlim = c(-0.1, 10), ylim = c(0, 0.8))
##D lines(xx, y)
##D with(fit, lines(xx, dlognormgpd(xx, lnmean, lnsd, u, sigmau, xi), col="red"))
##D abline(v = fit$u, col = "red")
##D   
##D # Parameterised tail fraction
##D fit2 = flognormgpd(x, phiu = FALSE)
##D with(fit2, lines(xx, dlognormgpd(xx, lnmean, lnsd, u, sigmau, xi, phiu), col="blue"))
##D abline(v = fit2$u, col = "blue")
##D legend("topright", c("True Density","Bulk Tail Fraction","Parameterised Tail Fraction"),
##D   col=c("black", "red", "blue"), lty = 1)
##D   
##D # Profile likelihood for initial value of threshold and fixed threshold approach
##D fitu = flognormgpd(x, useq = seq(1, 5, length = 20))
##D fitfix = flognormgpd(x, useq = seq(1, 5, length = 20), fixedu = TRUE)
##D 
##D hist(x, breaks = 100, freq = FALSE, xlim = c(-0.1, 10), ylim = c(0, 0.8))
##D lines(xx, y)
##D with(fit, lines(xx, dlognormgpd(xx, lnmean, lnsd, u, sigmau, xi), col="red"))
##D abline(v = fit$u, col = "red")
##D with(fitu, lines(xx, dlognormgpd(xx, lnmean, lnsd, u, sigmau, xi), col="purple"))
##D abline(v = fitu$u, col = "purple")
##D with(fitfix, lines(xx, dlognormgpd(xx, lnmean, lnsd, u, sigmau, xi), col="darkgreen"))
##D abline(v = fitfix$u, col = "darkgreen")
##D legend("topright", c("True Density","Default initial value (90% quantile)",
##D  "Prof. lik. for initial value", "Prof. lik. for fixed threshold"),
##D  col=c("black", "red", "purple", "darkgreen"), lty = 1)
## End(Not run)
  



