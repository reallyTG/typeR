...
### Name: forestFloor
### Title: Compute out-of-bag cross-validated feature contributions to
###   visualize model structures of randomForest models.
### Aliases: forestFloor forestFloor_randomForest_regression
...
##D   ntree = if(interactive()) 500 else 50 #speedup CRAN testing
##D )
##D 
...
##D vars = 15
##D noise_factor = .3
##D bankruptcy_baserate = 0.2
##D X = data.frame(replicate(vars,rnorm(N)))
##D y.signal = with(X,X1^2+X2^2+X3*X4+X5+X6^3+sin(X7*pi)*2) #some non-linear f
##D y.noise = rnorm(N) * sd(y.signal) * noise_factor
##D y.total = y.noise+y.signal
##D y = factor(y.total>=quantile(y.total,1-bankruptcy_baserate))
##D 
##D set.seed(1)
##D caret_train_obj <- train(
##D   x = X, y = y, 
##D   method = "rf",
##D   keep.inbag = TRUE,  #always set keep.inbag=TRUE passed as ... parameter to randomForest
##D   ntree=50, #speed up this example, if set too low, forestFloor will fail
##D  )
##D 
##D rf = caret_train_obj$finalModel #extract model
##D if(!all(rf$y==y)) warning("seems like training set have been resampled, using smote?")
##D ff = forestFloor(rf,X,binary_reg = T)
##D 
##D #... or simply pass train
##D ff = forestFloor(caret_train_obj,X,binary_reg = T)
##D plot(ff,1:6,plot_GOF = TRUE)
##D 
##D 
## End(Not run)
