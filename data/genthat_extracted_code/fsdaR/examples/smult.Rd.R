...
##D 
##D  out1 <- smult(Xcont, trace=TRUE)           # no plots (plot defaults to FALSE)
##D  names(out1)
##D 
...
##D 
##D  (out1 <- smult(Xcont, trace=TRUE, plot=TRUE))
##D 
##D  ## plots is a list: the spm shows the labels of the outliers.
##D  (out1 <- smult(Xcont, trace=TRUE, plot=list(labeladd="1")))
##D 
##D  ## plots is a list: the spm uses the variable names provided by 'nameY'.
##D  (out1 <- smult(Xcont, trace=TRUE, plot=list(nameY=c("A", "B", "C"))))
##D 
##D  ## smult() with monitoring
##D  (out2 <- smult(Xcont, monitoring=TRUE, trace=TRUE))
##D  names(out2)
##D 
...
##D 
##D  (out1 <- smult(swissbanknotes[101:200,], plot=TRUE))
##D 
##D  (out1 <- smult(swissbanknotes[101:200,], plot=list(labeladd="1")))
##D  
## End(Not run)
