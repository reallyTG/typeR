...
### Name: FMP
### Title: Estimate the coefficients of a filtered monotonic polynomial IRT
###   model
### Aliases: FMP
### Keywords: statistics
...
## Not run: 
##D ## In this example we will generate 2000 item response vectors 
##D ## for a k = 1 order filtered polynomial model and then recover 
##D ## the estimated item parameters with the FMP function.  
##D 
##D k <- 1  # order of polynomial
##D 
##D NSubjects <- 2000
##D 
##D 
##D ## generate a sample of 2000 item response vectors 
##D ## for a k = 1 FMP model using the following
##D ## coefficients
##D b <- matrix(c(
...
##D 
##D ## number of items in the data matrix
...
##D   for(i in 1:NItems){
##D     out <- FMP(data = ex1.data, thetaInit, item = i, startvals = startVals, k = k)
##D     Nb <- length(out$b)
##D     bmat[i,1:Nb] <- out$b
