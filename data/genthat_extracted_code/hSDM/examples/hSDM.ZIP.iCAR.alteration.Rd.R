...
### Name: hSDM.ZIP.iCAR.alteration
### Title: ZIP (Zero-Inflated Poisson) model with CAR process taking into
###   account site alteration
### Aliases: hSDM.ZIP.iCAR.alteration
...
###   Carlo Metropolis algorithm ZIB models Zero-Inflated Binomial models
...
##D 
##D #== Data-set
##D Data <- data.frame(Y,cells,X1,X2,U)
##D ## Uncomment if you want covariates on the observability process 
##D ## Data <- data.frame(Y,cells,X1,X2,W1,W2,U)
...
##D 
##D mod.hSDM.ZIP.iCAR.alteration <- hSDM.ZIP.iCAR.alteration(counts=Data$Y,
##D                                                suitability=~X1+X2,
##D                                                abundance=~1,
...
##D summary(mod.hSDM.ZIP.iCAR.alteration$mcmc)
##D 
##D #= MCMC and posteriors
##D pdf(file="Posteriors_hSDM.ZIP.iCAR.alteration.pdf")
##D plot(mod.hSDM.ZIP.iCAR.alteration$mcmc)
##D dev.off()
##D 
##D pdf(file="Posteriors.rho_hSDM.ZIP.iCAR.alteration.pdf")
##D plot(mod.hSDM.ZIP.iCAR.alteration$rho.pred)
##D dev.off()
##D 
##D #= Summary plots
##D 
...
##D r.rho[] <- rho
##D rho.pred <- apply(mod.hSDM.ZIP.iCAR.alteration$rho.pred,2,mean)
##D r.rho.pred[] <- rho.pred
##D r.visited[] <- 0
##D r.visited[visited.cells] <- tapply(Data$Y,Data$cells,mean)
##D # prob.p
##D r.prob.p <- Landscape
##D r.prob.p[] <- mod.hSDM.ZIP.iCAR.alteration$prob.p.pred
##D 
##D pdf(file="Summary_hSDM.ZIP.iCAR.alteration.pdf")
##D par(mfrow=c(3,2))
