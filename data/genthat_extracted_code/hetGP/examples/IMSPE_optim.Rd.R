...
##D 
##D for(i in 1:nsteps){
##D   res <- IMSPE_optim(model, h = 3, control = list(multi.start = 100, maxit = 50))
##D   
...
##D   
##D   ## Precalculations
##D   Wijs <- Wij(mu1 = model$X0, theta = model$theta, type = model$covtype)
##D   
##D   IMSPE_grid <- apply(Xgrid, 1, crit_IMSPE, Wijs = Wijs, model = model)
##D   filled.contour(x = xgrid, y = xgrid, matrix(IMSPE_grid, ngrid),
