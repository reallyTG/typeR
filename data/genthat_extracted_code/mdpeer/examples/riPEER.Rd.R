...
### Name: riPEER
### Title: Graph-constrained regression with penalty term being a linear
###   combination of graph-based and ridge penalty terms
### Aliases: riPEER
...
## Not run: 
##D riPEER.out <- riPEER(Q, y, Z, X)
##D plt.df <- data.frame(x = 1:p, y = riPEER.out$b.est)
##D ggplot(plt.df, aes(x = x, y = y, group = 1)) + geom_line() + labs("b estimates")
## End(Not run)

## Not run: 
##D # riPEER with 0.95 bootstrap confidence intervals computation
##D riPEER.out <- riPEER(Q, y, Z, X, compute.boot.CI = TRUE, boot.R = 500)
##D plt.df <- data.frame(x = 1:p, 
##D                      y = riPEER.out$b.est, 
##D                      lo = riPEER.out$boot.CI[,1], 
##D                      up =  riPEER.out$boot.CI[,2])
