library(pMineR)


### Name: cluster_partitionAroundMedoids
### Title: A class to perform Partition Around Medoids clustering on
###   sequential data for Process Mining issues
### Aliases: cluster_partitionAroundMedoids

### ** Examples

## Not run: 
##D 
##D # create a Loader 
##D obj.L<-dataLoader();   
##D 
##D # Load a .csv using "DES" and "ID" as column names to indicate events
##D # and Patient's ID
##D obj.L$load.csv(nomeFile = "../otherFiles/test_02.csv",
##D IDName = "ID",EVENTName = "DES",dateColumnName = "DATA")
##D 
##D # now create an object cluster_partitionAroundMedoids
##D obj.clPAM<- cluster_partitionAroundMedoids();
##D 
##D # load the data into logInspector object
##D obj.clPAM$loadDataset( obj.L$getData() );
##D 
##D # perform clustering computation
##D obj.clPAM$calculateClusters(num = 2);
##D 
##D # get calculated clusters
##D a <- obj.clPAM$getClusters();
##D 
##D # get informations about performance of clusters
##D b <- obj.clPAM$getClusterStats();
##D 
##D # get log of each iteration of the algorithm
##D d <- obj.clPAM$getClusterLog();
## End(Not run)



