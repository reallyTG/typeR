...
### Name: v_outer
### Title: Vectorized Version of outer
### Aliases: v_outer v_outer.list v_outer.data.frame v_outer.matrix
...
## Not run: 
##D library(qdap)
##D wc3 <- function(x, y) sum(sapply(list(x, y), wc, byrow = FALSE))
##D L1 <- word_list(DATA$state, DATA$person)$cwl
##D (x <- v_outer(L1, wc3))
##D diag(x) <- (sapply(L1, length))
##D x
##D 
