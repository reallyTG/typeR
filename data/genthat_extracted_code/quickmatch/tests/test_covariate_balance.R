...
replica_covariate_balance <- function(treatments,
                                      covariates,
                                      matching,
                                      normalize,
                                      target = NULL) {
  ...
  max_diffs <- c("1" = max(abs(diff_mats[["1"]]), na.rm = TRUE),
                 ...
                 "3" = max(abs(diff_mats[["3"]]), na.rm = TRUE),
...
get_sd_t1 <- function(in_cov) {
  sqrt(((sum(treatment1 == "A") - 1) * var(in_cov[treatment1 == "A"]) / sum(treatment1 == "A") +
          (sum(treatment1 == "B") - 1) * var(in_cov[treatment1 == "B"]) / sum(treatment1 == "B")) / 2)
}

get_sd_t2 <- function(in_cov) {
  sqrt(((sum(treatment2 == "A") - 1) * var(in_cov[treatment2 == "A"]) / sum(treatment2 == "A")  +
          (sum(treatment2 == "B") - 1) * var(in_cov[treatment2 == "B"]) / sum(treatment2 == "B") +
          (sum(treatment2 == "C") - 1) * var(in_cov[treatment2 == "C"]) / sum(treatment2 == "C")) / 3)
}

norm_treat1 <- c(get_sd_t1(cov[, 1]),
                 get_sd_t1(cov[, 2]),
                 get_sd_t1(cov[, 3]),
                 get_sd_t1(cov[, 4]),
                 get_sd_t1(cov[, 5]))

norm_treat2 <- c(get_sd_t2(cov[, 1]),
                 get_sd_t2(cov[, 2]),
                 get_sd_t2(cov[, 3]),
                 get_sd_t2(cov[, 4]),
                 get_sd_t2(cov[, 5]))
...
test_that("`covariate_balance` no matching", {
  expect_equal(covariate_balance(treatment1, cov, normalize = TRUE, all_differences = TRUE),
               replica_covariate_balance(treatment1, cov, test_matching, norm_treat1))
  expect_equal(covariate_balance(treatment1, cov, normalize = TRUE, all_differences = FALSE),
               replica_covariate_balance(treatment1, cov, test_matching, norm_treat1)$max_diffs)
  expect_equal(covariate_balance(treatment1, cov, normalize = FALSE, all_differences = TRUE),
               replica_covariate_balance(treatment1, cov, test_matching, rep(1, 5)))
  expect_equal(covariate_balance(treatment1, cov, normalize = FALSE, all_differences = FALSE),
               replica_covariate_balance(treatment1, cov, test_matching, rep(1, 5))$max_diffs)

  expect_equal(covariate_balance(treatment2, cov, normalize = TRUE, all_differences = TRUE),
               replica_covariate_balance(treatment2, cov, test_matching, norm_treat2))
  expect_equal(covariate_balance(treatment2, cov, normalize = TRUE, all_differences = FALSE),
               replica_covariate_balance(treatment2, cov, test_matching, norm_treat2)$max_diffs)
  expect_equal(covariate_balance(treatment2, cov, normalize = FALSE, all_differences = TRUE),
               replica_covariate_balance(treatment2, cov, test_matching, rep(1, 5)))
  expect_equal(covariate_balance(treatment2, cov, normalize = FALSE, all_differences = FALSE),
               replica_covariate_balance(treatment2, cov, test_matching, rep(1, 5))$max_diffs)
})
...
test_that("`covariate_balance` matching 1", {
  expect_equal(covariate_balance(treatment1, cov, test_matching, normalize = TRUE, all_differences = TRUE),
               replica_covariate_balance(treatment1, cov, test_matching, norm_treat1))
  expect_equal(covariate_balance(treatment1, cov, test_matching, normalize = TRUE, all_differences = FALSE),
               replica_covariate_balance(treatment1, cov, test_matching, norm_treat1)$max_diffs)
  expect_equal(covariate_balance(treatment1, cov, test_matching, normalize = FALSE, all_differences = TRUE),
               replica_covariate_balance(treatment1, cov, test_matching, rep(1, 5)))
  expect_equal(covariate_balance(treatment1, cov, test_matching, normalize = FALSE, all_differences = FALSE),
               replica_covariate_balance(treatment1, cov, test_matching, rep(1, 5))$max_diffs)
})
...
test_that("`covariate_balance` matching 2", {
  expect_equal(covariate_balance(treatment2, cov, test_matching, normalize = TRUE, all_differences = TRUE),
               replica_covariate_balance(treatment2, cov, test_matching, norm_treat2))
  expect_equal(covariate_balance(treatment2, cov, test_matching, normalize = TRUE, all_differences = FALSE),
               replica_covariate_balance(treatment2, cov, test_matching, norm_treat2)$max_diffs)
  expect_equal(covariate_balance(treatment2, cov, test_matching, normalize = FALSE, all_differences = TRUE),
               replica_covariate_balance(treatment2, cov, test_matching, rep(1, 5)))
  expect_equal(covariate_balance(treatment2, cov, test_matching, normalize = FALSE, all_differences = FALSE),
               replica_covariate_balance(treatment2, cov, test_matching, rep(1, 5))$max_diffs)
})
...
test_that("`covariate_balance` matching 2", {
  expect_warning(covariate_balance(treatment2, cov, test_matching, normalize = TRUE, all_differences = TRUE),
                 regexp = "Some matched groups are missing treatment conditions. Corresponding balance measures cannot be derived.")

  expect_warning(expect_equal(covariate_balance(treatment2, cov, test_matching, normalize = TRUE, all_differences = TRUE),
                              replica_covariate_balance(treatment2, cov, test_matching, norm_treat2)))
  expect_warning(expect_equal(covariate_balance(treatment2, cov, test_matching, normalize = TRUE, all_differences = FALSE),
                              replica_covariate_balance(treatment2, cov, test_matching, norm_treat2)$max_diffs))
  expect_warning(expect_equal(covariate_balance(treatment2, cov, test_matching, normalize = FALSE, all_differences = TRUE),
                              replica_covariate_balance(treatment2, cov, test_matching, rep(1, 5))))
  expect_warning(expect_equal(covariate_balance(treatment2, cov, test_matching, normalize = FALSE, all_differences = FALSE),
                              replica_covariate_balance(treatment2, cov, test_matching, rep(1, 5))$max_diffs))
})
