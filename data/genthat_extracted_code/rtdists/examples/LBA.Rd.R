...
### Aliases: LBA dLBA pLBA qLBA rLBA
...
## Not run: 
##D # can we recover the parameters? 
##D # should be run several times with different random values of init_par
##D init_par <- runif(6)
##D init_par[2] <- sum(init_par[1:2]) # ensures b is larger than A
##D init_par[3] <- runif(1, 0, min(rt1$rt)) #ensures t0 is mot too large
##D names(init_par) <- c("A", "b", "t0", "mean_v1", "mean_v2",  "sd_v2")
##D nlminb(objective_fun, start = init_par, rt=rt1$rt, response=rt1$response, lower = 0)
## End(Not run)
