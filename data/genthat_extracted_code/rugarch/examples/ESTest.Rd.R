...
##D # calculate ES
##D f = function(x) qdist("sstd", p=x, mu = 0, sigma = 1, 
##D skew  = coef(fit)["skew"], shape=coef(fit)["shape"])
##D ES = fitted(filt) + sigma(filt)*integrate(f, 0, 0.05)$value/0.05
##D print(ESTest(0.05, actual, ES, VaR, boot = TRUE))
## End(Not run)
