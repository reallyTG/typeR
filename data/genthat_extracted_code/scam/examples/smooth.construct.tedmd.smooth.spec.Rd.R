...
### Name: smooth.construct.tedmd.smooth.spec
### Title: Tensor product smoothing constructor for bivariate function
###   subject to double monotone decreasing constraint
### Aliases: smooth.construct.tedmd.smooth.spec
### Keywords: models regression

### ** Examples

 ## Not run: 
##D ## tensor product `tedmd' example 
##D   ## simulating data...
##D    set.seed(2)
##D    n <- 30
...
##D ## fit model ...
##D    b <- scam(y~s(x1,x2,k=c(10,10),bs="tedmd",m=2),
##D             family=gaussian(link="identity"), data=dat)
##D ## plot results ...
##D    par(mfrow=c(2,2),mar=c(4,4,2,2))
##D    plot(b,se=TRUE)
##D    plot(b,pers=TRUE,theta = 80, phi = 40)
