...
### Name: smooth.construct.temicv.smooth.spec
### Title: Tensor product smoothing constructor for bivariate function
###   subject to mixed constraints: monotone increasing constraint wrt the
###   first covariate and concavity wrt the second one
### Aliases: smooth.construct.temicv.smooth.spec
### Keywords: models regression

### ** Examples

 ## Not run: 
##D ## tensor product `temicv' example 
##D   ## simulating data...
##D 
##D    set.seed(2)
##D    n <- 30
...
##D        { f1[i,j] <- exp(4*x1[i])/(1+exp(4*x1[i])) - 4*x2[j]^2}    
##D    f <- as.vector(t(f1))
...
##D ## fit model ...
##D    b <- scam(y~s(x1,x2,k=c(10,10),bs="temicv",m=2), data=dat)
##D ## plot results ...
