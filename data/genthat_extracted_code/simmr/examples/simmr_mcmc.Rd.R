...
### Aliases: simmr_mcmc print.simmr_output
...
##D simmr_2 = simmr_load(mixtures=mix[1,,drop=FALSE], # drop required to keep the mixtures as a matrix 
##D                      source_names=s_names,
##D                      source_means=s_means,
##D                      source_sds=s_sds,
##D                      correction_means=c_means,
...
##D 
##D # The data
##D mix = matrix(c(-11.67, -12.55, -13.18, -12.6, -11.77, -11.21, -11.45, 
##D                -12.73, -12.49, -10.6, -12.26, -12.48, -13.07, -12.67, -12.26, 
##D                -13.12, -10.83, -13.2, -12.24, -12.85, -11.65, -11.84, -13.26, 
##D                -12.56, -12.97, -12.18, -12.76, -11.53, -12.87, -12.49, 7.79, 
##D                7.85, 8.25, 9.06, 9.13, 8.56, 8.03, 7.74, 8.16, 8.43, 7.9, 8.32, 
##D                7.85, 8.14, 8.74, 9.17, 7.33, 8.06, 8.06, 8.03, 8.16, 7.24, 7.24, 
##D                8, 8.57, 7.98, 7.2, 8.13, 7.78, 8.21, 11.31, 10.92, 11.3, 11, 
##D                12.21, 11.52, 11.05, 11.05, 11.56, 11.78, 12.3, 10.87, 10.35, 
##D                11.66, 11.46, 11.55, 11.41, 12.01, 11.97, 11.5, 11.18, 11.49, 
##D                11.8, 11.63, 10.99, 12, 10.63, 11.27, 11.81, 12.25), ncol=3, nrow=30)
##D colnames(mix) = c('d13C','d15N','d34S')
##D s_names = c('Source A', 'Source B', 'Source C', 'Source D') 
##D s_means = matrix(c(-14, -15.1, -11.03, -14.44, 3.06, 7.05, 13.72, 5.96, 
##D                    10.35, 7.51, 10.31, 9), ncol=3, nrow=4)
##D s_sds = matrix(c(0.46, 0.39, 0.42, 0.48, 0.44, 0.37, 0.49, 0.47, 0.49, 
##D                  0.42, 0.41, 0.42), ncol=3, nrow=4)
##D c_means = matrix(c(1.3, 1.58, 0.81, 1.7, 1.73, 1.83, 1.69, 3.2, 0.67, 
##D                    2.99, 3.38, 1.31), ncol=3, nrow=4)
##D c_sds = matrix(c(0.32, 0.64, 0.58, 0.46, 0.61, 0.55, 0.47, 0.45, 0.34, 
##D                  0.45, 0.37, 0.49), ncol=3, nrow=4)
##D conc = matrix(c(0.05, 0.1, 0.06, 0.07, 0.07, 0.03, 0.07, 0.05, 0.1, 
##D                 0.05, 0.12, 0.11), ncol=3, nrow=4)
##D 
##D # Load into simmr
...
##D 
##D # The data
##D mix = matrix(c(-14.65, -16.39, -14.5, -15.33, -15.76, -15.15, -15.73, 
##D                -15.52, -15.44, -16.19, 8.45, 8.08, 7.39, 8.68, 8.23, 7.84, 8.48, 
##D                8.47, 8.44, 8.37), ncol=2, nrow=10)
##D s_names = c('Source A', 'Source B', 'Source C', 'Source D') 
##D s_means = matrix(c(-25, -25, -5, -5, 4, 12, 12, 4), ncol=2, nrow=4)
##D s_sds = matrix(c(1, 1, 1, 1, 1, 1, 1, 1), ncol=2, nrow=4)
##D 
##D # Load into simmr - note no corrections or concentrations
##D simmr_4 = simmr_load(mixtures=mix,
##D                      source_names=s_names,
##D                      source_means=s_means,
##D                      source_sds=s_sds)
##D 
...
##D mcmc.control=list(iter=100000,burn=10000,thin=100,n.chain=4))
##D 
##D # Print it
##D print(simmr_4_out)
##D 
...
##D plot(simmr_4_out,type='density') # Look at the massive correlations here
##D plot(simmr_4_out,type='matrix')
##D 
##D #####################################################################################
##D 
...
##D 
##D # Load this in:
##D simmr_5 = simmr_load(mixtures=mix,
##D                      source_names=s_names,
##D                      source_means=s_means,
##D                      source_sds=s_sds,
##D                      correction_means=c_means,
...
##D 
##D # Plot
##D plot(simmr_5,group=1:8,xlab=expression(paste(delta^13, "C (\u2030)",sep="")), 
...
##D compare_sources(simmr_5_out,sources=c('Zostera','U.lactuca'),group=2)
##D compare_sources(simmr_5_out,group=2)
##D 
##D # Compare between groups
##D compare_between_groups(simmr_5_out,source='Zostera',groups=1:2)
##D compare_between_groups(simmr_5_out,source='Zostera',groups=1:3)
##D compare_between_groups(simmr_5_out,source='U.lactuca',groups=c(4:5,7,2))
##D 
##D 
## End(Not run)
