...
test_that("color columns not existing", {
  result <- vistime:::set_colors(data.frame(
    event = 1:4, start = c("2019-01-01", "2019-01-10"),
    end = c("2019-01-01", "2019-01-10")
  ), eventcolor_column = "NOTEXISTING", fontcolor_column = "NOTEXISTING")
...
test_that("eventcolor column existing", {
  result <- vistime:::set_colors(data.frame(
    ...
    FARBE = "red", stringsAsFactors = F
  ), eventcolor_column = "FARBE", fontcolor_column = "NOTEXISTING")
  expect_true(all(c("col", "fontcol") %in% names(result)))
...
test_that("fontcolor column existing", {
  result <- vistime:::set_colors(data.frame(
    ...
    FARBE = "red", stringsAsFactors = F
  ), eventcolor_column = "NOTEXISTING", fontcolor_column = "FARBE")
  expect_true(all(c("col", "fontcol") %in% names(result)))
