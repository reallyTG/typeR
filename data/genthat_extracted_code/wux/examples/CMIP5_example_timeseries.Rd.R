library(wux)


### Name: CMIP5_example_timeseries
### Title: Climate change signals of example userinput for models2wux
### Aliases: CMIP5_example_timeseries
### Keywords: datasets

### ** Examples

## thats what CMIP5_timeseries looks like
data("CMIP5_example_timeseries")
head(CMIP5_example_timeseries)

## You can run models2wux to get the same result as
## above.
data(userinput_CMIP5_timeseries)
data(modelinput_test)
## Not run: 
##D ## You must have downloaded the example NetCDF files according to
##D ## "modelinput_test" in order to run "models2wux". See the examples of
##D ## ?CMIP5fromESGF or ?modelinput_test.  
##D CMIP5_example_timeseries <- models2wux(userinput_CMIP5_timeseries,
##D                                 modelinput = modelinput_test)
## End(Not run)




