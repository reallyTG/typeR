...
mutate(percentage = round(100 * parameter_count / sum(parameter_count), 2),
       cumulative_percentage = round(100 * cumsum(parameter_count) / sum(parameter_count), 2))
...
summarize(type = paste0(sort(type), collapse=" | ")) %>%
ungroup() %>%
group_by(alternative_count, type) %>%
summarize(count = n()) %>%
ungroup() %>%
group_by(alternative_count) %>%
arrange(desc(count)) %>%
...
group_by(alternative_count) %>%
mutate(percentage = round(100 * count / sum(count), 2),
       cumsum_percentage = round(100 * cumsum(count) / sum(count), 2)) %>%
ungroup() %>%
group_by(alternative_count) %>%
slice(1:20) %>%
ungroup()
