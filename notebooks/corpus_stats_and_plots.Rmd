...
count_df <- readRDS("../data/final_loc_counts.RDS")
...
plot_both <- plot_both + theme(
  ...
  # scale_x_discrete(name="Package", breaks = unlist(loc_of_top_10[!(loc_of_top_10 == FALSE)]), labels = top_10)
...
plot_records <- ggplot(data=records_by_fun, aes(x=pkg, y=invo_obs, group=1)) +
                geom_line(size=0.25) + 
                labs( title = "Numbers of Observed Signatures",
                      y = "Observed Invocations in Pkg (Thousands, log10-Scaled)") +
