...
```{r}
# read in new one
psp_L0 <- read_csv("../data/partial_L0_subtype.csv.gz")
p
# remove SUBTYPE rows
...
```{r}
# read it back
psp <- read_csv("../data/partial_L1_subtype.csv.gz")
...
```{r}
# ...
list_of_L2_sig_c <- read_csv("../data/partial_L1_arg_sig_counts.csv.gz")
...
# Arg Poly Count L2
data.frame(`Signature Type`=c("Monomorphic", "Total Seen"), Count=c(list_of_L2_sig_c$count %>% sum - L2_poly_sig$count %>% sum, list_of_L2_sig_c$count %>% sum), Percentage=c(paste0(round(100 * (1 - (L2_poly_sig$count %>% sum / list_of_L2_sig_c$count %>% sum)), 1), "%"), "---")) %>% xtable %>% print(include.rownames=F)
```
...
poly_count_normalized %>% arrange(-poly_norm) %>% slice(2:6)
