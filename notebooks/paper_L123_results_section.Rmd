...
# L <: I <: D
los <- lapply(los, function(lofun) lapply(lofun, logical_int_double_subtype_df))
```
...
L0_poly_type_args$perc <- round(L0_poly_type_args$count/sum(L0_poly_type_args$count) * 100, 2)
```

Step 2.3: Print top 10.
```{r}
format_df_for_print(L0_poly_type_args[1:10,]) %>%
...
```{r}
format_df_for_print(L3_poly_class_args[1:10,]) -> put_me
put_me
xtable(put_me) %>% print(include.rownames=F)
```
