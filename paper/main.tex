...
Designing a type system for R is no small feat, as statically making sense of R code is complicated by a language design which fosters an interactive programming style by minimizing programmer overhead.
Our intent is to eventually propose changes to the language, but we are aware that for any changes to be accepted by the community, they must have clear benefits without endangering backwards compatibility.
...
To this end, we designed a simple type language which found a compromise between simplicity and usefulness.
Our language focuses on the most widely used features of R and omits features such as parametric polymorphism and user-defined subtyping.
...
Retrofitting a type system for the interactive and exploratory programming style
of R is hard. It is even challenging to gain acceptance by the user community.
To this end, we have begun our journey with a simple type language, one that we
believe captures the essence of R while remaining simple and useful. We focus on
the basic language features for generating the type signatures, and find that
