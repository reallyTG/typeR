...
types.
 ...
 \AT{Kinds of failures: vectors passed to scalars, wrong things passed to classes, nulls passed to non-nullable arguments.}
 
 We observed a number of other interesting patterns in the contract assertion failure data.
 
 First, we were curious about how many \null-typed values flowed into non-nullable arguments.
 We found that \AT{fill in numbers}.
 Of these, there were three main kinds of ``type errors''.
 
 \begin{itemize}
   \item One kind of type error occurs when an argument with a {\tt NULL} default value is not given a value by the programmer.
   In testing, the programmer only tested the function by passing values to these null-by-default arguments.
   \AT{Check correlation with coverage.}
   
   \item Another kind of type error occurs when {\tt NULL} is passed to an argument which is not expected to be {\tt NULL}, but that does not cause an error.
   One example is \AT{see as.integer example}.
   
   \item Finally, in some cases the {\tt NULL}s resulted in errors, either unhandled or handled by a check and a conditional.
 \end{itemize}
