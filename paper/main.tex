...
\newcommand{\reals}{$real$\xspace}
\newcommand{\realv}{$\overline{real}$\xspace}
\newcommand{\chars}{$char$\xspace}
\newcommand{\charv}{$\overline{char}$\xspace}
\newcommand{\logis}{$logical$\xspace}
\newcommand{\logiv}{$\overline{logical}$\xspace}
\newcommand{\dbls}{$double$\xspace}
\newcommand{\dblv}{$\overline{double}$\xspace}
\newcommand{\ints}{$integer$\xspace}
\newcommand{\intv}{$\overline{integer}$\xspace}
...
aligns with R basic types. Richers notions incorporate attributes and the
various object systems built on top of those. For concreteness we will pick
out three different sets of types.

\begin{itemize}
\item {\bf L0}: This level includes vectors of primitive types (integers \I,
  double \D, character \C, logical \L, raw \R, complex \X), lists of any
  type (\l), and scalar values (environment \sE, closures \sF, symbols \sS
  and null \sN). We add the type \ANY for any type.
\item {\bf L1}: The second level enhances {\bf L0} with scalar values for
  primitive types (\sI, \sD, \sC, \sL, \sR, and \sX), type parametric lists
  (written \lT{T} for any type {\tt T}), data frames \attr{\l}{df}, and
  matrices \attr{\vec{\tt T}}{mat}.
\item {\bf L2}: The third level adds attributes lists...
\end{itemize}
...
\end{lstlisting}}\caption{Example of a data.frame/(\dblv) argument (from {\tt klausR} package).}\label{fig:dfdbl}\end{figure}
...
tee <- function (x, theta, D1, D2, phi)  {
    ...
    ...
\end{lstlisting}}\caption{Example of matrix/(\dblv) argument (from {\tt calibrator} package).}\label{fig:matvec}\end{figure}
...
From Figure~\ref{fig:L1argcounts}, we glean that argument positions aren't
...
x & \dbls & numeric & \{\} \\
y & \dblv & numeric & \{\} \\
retv & list<double> & list & \{\xspace example: \chars\}  
\end{tabular}
...
We note that an argument which exhibits a signature including e.g. \{\dbls, \dblv\} really must be an argument designed to take vectors, as recall that unit length vectors appear as scalars by our definition.
...
In L2, we aim to incorporate R's other reflective capabilities to address some of the more subtle instances of polymorphism.

We observe that the act of ascribing a class to a value in R carries the intent of the programmer.
Now, not all classes are user-defined, and R has a number of built-in or otherwise standard classes, ranging from truly primitive classes such as ``numeric'' to the more complex ``data.frame''.
Thus, before considering the insights provided by the class of values, we transform each of R's standard classes into a ``primitive'' class, which will serve to highlight user-defined classes and those particular to some packages.

\AT{I don't know the best way to approach this subsection, so I'm just gonna start looking at some stuff and see where it leads.}

First, we consider the purpose of classes in R.
The 

As before, we start with the counts, see Table~\ref{tab:classcountsL2}.
...
Type & Count & Percentage \\ 
  \hline
Full Monomorphic & 777 & 84.96 \\ 
%  Monomorphic in Type & 869.00 & 95.01 \\ 
  Monomorphic in Class & 850 & 92.94 \\ 
%  Monomorphic in Attribute Pattern & 825.00 & 90.27 \\ 
  Total Seen & 914 & --- \\ 
   \hline
\end{tabular}
\caption{Account of class-based polymorphism in L2.}
\end{table}

Top signatures in Table~\ref{tab:classpolyL2}.
...
primitive, tbl, tbl\_df & 1796 & 2.78 \\ 
  ...
  xts, zoo & 588 & 0.91 \\ 
  glm, lm & 494 & 0.77 \\ 
  primitive, ts & 432 & 0.67 \\ 
  mts, primitive, ts & 384 & 0.59 \\ 
  huxtable, primitive & 318 & 0.49 \\ 
   \hline
\end{tabular}
\caption{Top 10 signatures for class-based polymorphism in L2.}
\end{table}

Given the low percentages overall, we see that there are no clear patterns here.
This data does highlight some common classes across R packages:
tbl and tbl\_df are from the dplyr package, as classes for data objects.
data.table is the primary data type in the data.table package, again for data objects.
POSIXct and POSIXt are for dates.
...
\begin{tabular}{llrl}
  ...
  list$<$any$>$, list$<$real$>$ & phylo & 23 & 1.69 \\ 
  list$<$any$>$, list$<$list$>$, list$<$real$>$, scalar/real & dendrogram & 21 & 1.55 \\ 
  list$<$any$>$, list$<$language$>$ & uneval & 18 & 1.33 \\ 
  list$<$any$>$, list$<$list$>$ & dendrogram & 18 & 1.33 \\ 
  list$<$any$>$, list$<$list$>$ & lazy\_dots & 17 & 1.25 \\ 
   \hline
\end{tabular}
\end{table}

\AT{I don't think this table is telling us much...}

Top attributes, maybe?

% latex table generated in R 3.5.1 by xtable 1.8-3 package
% Mon Apr  1 19:21:13 2019
\begin{table}[ht]
\centering
\begin{tabular}{lrl}
  \hline
attr & count & perc \\ 
  \hline
\{\}, \{\xspace names: \charv\} & 9274.00 & 10.43 \\ 
  \{\}, \{\xspace names: \chars\} & 4900.00 & 5.51 \\ 
  \{\}, \{\xspace dim: \intv\} & 4758.00 & 5.35 \\ 
  \{\xspace names: \chars\}, \{\xspace names: \charv\} & 3457.00 & 3.89 \\ 
  \{\}, \{\xspace names: \chars\}, \{\xspace names: \charv\} & 2415.00 & 2.72 \\ 
  \{\xspace dim: \intv, dimnames:list$<$any$>$ \}, \{\xspace dim:\intv\} & 2264.00 & 2.55 \\ 
  \{\}, \{\xspace srcref: \intv\} & 2037.00 & 2.29 \\ 
  \{\xspace dim: \intv, dimnames: list$<$character$>$\}, \{ dim:\intv\} & 1734.00 & 1.95 \\ 
  \{\xspace class: \chars, names: \charv, row.names:\charv\}, \{class:\chars, names:\charv, row.names:\intv\} & 1464.00 & 1.65 \\ 
  \{\xspace class: \chars, names: \charv\}, \{ class:\charv, names:\charv\} & 1277.00 & 1.44 \\ 
   \hline
\end{tabular}
\caption{Top attribute patterns. \AT{TODO fix this up}}
\end{table}\charv
