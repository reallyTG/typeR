...
\AT{Note: some stuff I wrote that might belong somewhere in here, if not, delete.}
...
\AT{I wrote a paragraph in the next section which seems to belong here, maybe work this in?}
...
% > talk about CRAN
% > what is a package 
% > look at genthat paper for inspiration
% > communicate the size of the adventure
% > convince people that this is real
% > lines of C and R code per project? big graph could be nice
% > give people a sense of our data set
...
\AT{Do we want an example of a vignette? We could put one.}
...
Most R scripts are not built to be exported, and are part of an iterative and interactive process of data analysis, and unlike other programming languages, R has a very small footprint on GitHub, and most of the R code available there is referenced through CRAN \AT{maybe we can get numbers here? or just take out the sentence}.
...
\AT{Not sure how to word the second half of this paragraph.}
In focusing on these exemplary snippets, we should capture the intent of package designers, which gives up hope that our proposed type annotations would be adopted.
...
We ran our analysis over a subset of CRAN, analyzing \PACKAGES packages, account for some \PERCENTCRAN of all packages available on CRAN.
\AT{Talk about timeout?}  Due to the nature of some packages (e.g., packages implementing a computationally-intensive model with many tests), we ran our analysis with a 1 hour timeout.
...
\begin{itemize}
	...
	{\tt ggplot2}'s role in the tidyverse is to make creating elegant plots easy: in it, graphics are created declaratively in a manner based on The Grammar of Graphics \AT{cite}.
