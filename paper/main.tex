...
\paragraph{Nullables} 
The type system has a \null type inhabited by a singleton \code{NULL}
value. This value is often used as a sentinel or as a stand in for arguments
that were not provided.  To capture this behavior, we introduce a nullable
type \code{?}\T.  The difference between the \NA and \code{NULL} values is
that \NA can be stored in primitive vectors.
