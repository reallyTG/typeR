...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Design A Type Language for R}%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\renewcommand{\null}{\texttt{\textbf{null}}\xspace}
\newcommand{\any}{\texttt{\textbf{any}}\xspace}
\newcommand{\environment}{\texttt{\textbf{environment}}\xspace}
\newcommand{\expression}{\texttt{\textbf{expression}}\xspace}
\newcommand{\Language}{\texttt{\textbf{language}}\xspace} %% Upper case!
...
\T& ::= & \null         & \it unit type\\
  ...
  & |   & \Language     & \it language type \\
  & |   & \symbol       & \it symbol type \\
  & |   & \S            & \it scalar type \\
  & |   & \V            & \it vector type \\
  & |   & \T~\k{|}\T    & \it union type \\
  & |   & \k{?} \T      & \it nullable type \\
  & |   & \FUN{A_1, ..., A_n}\T & \it function type \\
  & |   & \STRUCT{F_1, ..., F_n} & \it struct type \\
  & |   & \LIST{T}      & \it list type\\
...
\A & ::= & \T & \it arguments \\
  & |   & \VARGS \vspace{5pt} \\
\F & ::= & \ID\, :\,T & \it struct field\vspace{5pt}\\
\V & ::= & ~\VEC\S &\\
...
The workhorse types in R are the vectorized primitives: integers, doubles,
complex numbers, logicals (booleans), characters, and raws (bytes).  These
are vectorized in that they are always considered to be vectors by the R
...
scalars and often do high-level
...
R has more than one notion of type.  The classes of function arguments are
used by R as part of its dispatch mechanism, the most prevalent of which
(\AT{we have that number, too!}) is S3 dispatch, in which R dispatches on
the (S3) class of the first argument to a function, selecting the
appropriate method to call for the class. 
...
analysis and allowed us to gather more focused data. 

When set to ignore classes, \typetracer reports \AT{type} as the type of the
function, as compared with \AT{other type} when configured to report
classes.
%
