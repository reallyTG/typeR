...
\begin{itemize}
	...
	\item if the analysis encounters \code{ typeof(x) == "double"}, it will look at the length of {\tt x} to determine if {\tt x} is a scalar or a vector, generating the annotation {\tt \dbls} or {\tt \dblv} accordingly.
...
Some entries in the Table~\ref{tab:L0toppoly} are clear (e.g., double, integer polymorphism is unsurprising), others are strange (e.g., character, double), but in fact they all have some explanation.
