...
contexts a \k{logical} will convert to \k{integer}, and an \k{integer} will
...
these \k{jumped} become \any types.
...
limited to twenty arguments as a simplifying assumption. Higher-order functions are approximated by \any
to \any by \typetracer.
...
require checking for each of the alternatives. As the usage
...
This occurs in only 35.90\% of such arguments,
...
\CLASS{{\tt dtplyr\_step\_first, dtplyr\_step}} & \CLASS{{\tt data.frame}} | Grouped Tibble | Tibble & 10 & 45 & 22.22 \\
\chr & \raw & 6 & 31 & 19.35 \\
...
First, we observe a class of errors related to ``subclasses'' being passed to arguments expecting a ``superclass''.
For instance, we observed \CLASS{{\tt data.frame}} values being passed to arguments expecting tibbles or data.tables (which can be readily converted).
...
\midrule
 ...
 \VEC\chr & \LIST{\VEC\int} $|$ ... $|$ \LIST{\CLASS{{\tt formula, quosure}}} & 5385 & 7.58 & 72.12 \\ 
 \CLASS{{\tt ixforeach, iter}} & \CLASS{{\tt dataframeiter, iter}} $|$ ... $|$ \CLASS{{\tt iforeach, iter}} & 4139 & 5.82 & 77.94 \\ 
\bottomrule
\end{tabular}
...
\bottomrule
\end{tabular}
\caption{What is the highest failure rate among popular argument types in Kaggle? For argument signatures whose frequency is in the 90th percentile}
...
\end{table}
 ...
 While we don't see many overlapping entries per se, the table entries have similar explanations.
 For instance, values of type \CLASS{{\tt data.table, data.frame}} are often passed to arguments expecting a \CLASS{{\tt matrix}}.
 \CLASS{{\tt data.table, data.frame}} values are known as ``data tables'', and essentially serve the same purpose as data frames and tibbles.
 As it happens, many data tables can be coerced to matrices, and programmers will often interchange the two (particularly when data tables have numeric data), as is the case here.
...
class::knn <- function (train, test, cl, k = 1, l = 0, prob = FALSE, use.all = TRUE)  {
 ...
 \end{lstlisting}
 We see that \code{class::knn} can take arguments as long as they can be passed to \code{as.matrix}, e.g., data tables, and will perform the coercion.
 As in similar cases discussed earlier, we did not encode this semantics in our type system as the process of coercion in R is ad hoc.
 ...
 \CLASS{{\tt factor}} typed values are known as factors in R, and they are stored as a vector of integer values corresponding to a set of character values, and their purpose is to allow for quickly deal with categorized data.
 ...
 This is likely an instance where the user defined their own class, {\tt ixforeach}, and wanted to use the \code{iterators} package (the user called \code{iterator::iter} with a \CLASS{{\tt ixforeach}}, and it gained the class tag {\tt iter} on return).
