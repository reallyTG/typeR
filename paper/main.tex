...
$$type `data.tree::Traverse` \FUN{\CLASS{`Node`, `R6`}, ~\VEC\chr, \any, \any}\LIST\any;$$

$$type `openssl::decrypt\_envelope` \FUN{~\VEC\raw, ~\VEC\raw, ~\VEC\raw, \CLASS{`key`, `rsa`}, \any}~\VEC\raw;$$

$$dbplyr:type `set\_win\_current\_group` \FUN{? ~\VEC\chr}? ~\VEC\chr;$$

$$type `sha256` \FUN{~\VEC\raw, ? ~\VEC\raw}~\VEC\raw;$$

$$type `initparam` \FUN{? \dbl, \any, \any, \any, \chr, \chr, \lgl, ~\VEC\dbl, ~\VEC\dbl, \any}~\VEC\dbl;$$
