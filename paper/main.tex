...
that an explicit matrix should be passed. We captured this with our class
types, and indeed \typetracer inferred a good type for this function,
allowing \contractr to perform the check present in this code automatically,
notifying the users passing ill-typed arguments.
...
pdiscussed previously.  Here, the length of the weights vector \code{w} is
...
In R, lists can have a names attribute, allowing lists elements to be
accessed by name using the \code{$} operator.
...
\code{x}, which we observed to always be a {\it linear model} (either ANOVA
with \CLASS{aov, lm} or a simple linear regression model with \CLASS{lm})
which is part of R's built-in statistics functionality.  Internally, linear
models are represented as lists with named elements, and one may access
these named elements using the \code{$} syntax, as seen on lines 2-4 in the
snippet.  To describe these kinds of values, we intended to include a struct
type in our type system, allowing users to specify name, type pairs as part
of list type definitions, though we found this to be problematic.

The inclusion of structs complicated our analysis by polluting the data,
illustrated on line 8 with the call \code{data(sweetpotato)}.  It is quite
common for R example code to use some built-in data sets, such as
\code{sweetpotato}, to show off some package functionality with ``real
data''.  When \typetracer is configured to detect structs we saw a marked
increase in misrepresentative types, as the types of functions which use
these data sets (such as \code{aov} in the code example) became polluted
with the names from the example sets.  We implemented a unification strategy
for struct types, removing them if they co-occurred with list types, and
turning them into lists if two co-occurring struct types shared no common
names, but even so we found ourselves with function signatures that were
full of noise introduced by the struct names.

We found that struct-like data was best described with classes, as
illustrated in the code snippet, and many structs actually had a class.
Even though \code{x} is used like a struct, it in fact has a linear model
class, which we believe is a more informative type.
...
type `data.tree::Traverse` <class<`Node`, `R6`>, character[], any, any> => list<any>;

type `emulator::cprod` <class<`matrix`> | double[], class<`matrix`> | double[]> => class<`matrix`>;

type `lava::lava.options` <...> => list<double | character[]>;

type `coin::\%GE\%` <class<`matrix`> | class<`matrix`, `pvalue`> | double[], double[] | class<`matrix`, `pvalue`>> => (class<`matrix`> | logical[]);

type `emplik::llogp` <class<`matrix`> | double[], double> => (class<`matrix`> |
double[]);

type `emplik::gradf` <class<`matrix`>, double[], double[]> => double[];

type `fields::rdist` <class<`data.frame`> | class<`matrix`> | double[], class<`data.frame`> | class<`matrix`> | null | double[], logical> => (class<`matrix`> | class<`dist`>);

type `stabledist::x.exp.m.x` <double[]> => double[];

type `lava::index` <class<`lvm`> | class<`lvmfit`>, ...> => (null | list<any>);

type `pkgload::compact` <list<any>> => list<any>;

type `diptest::dip` <double[], character | logical, logical, double> => (class<`dip`> | double);


type `bbmle::relist2` <double[], double[] | list<any>> => list<any>;

type `stabledist::cospi2` <double[]> => double[];
type `stabledist::tanpi2` <double[]> => double[];

type `neuralnet::relist` <double[], class<`array`> | integer[], class<`array`> | integer[]> => list<class<`matrix`>>;
type `neuralnet::calculate.gradients` <any, integer, list<class<`matrix`>>, any, class<`matrix`>, double, logical> => double[];
