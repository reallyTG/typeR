...
value. The R internal types are translated to the type name of the proposed
type system. The next bit of information output for each value is its
\emph{class}.  The class is an optional list of names, it may be absent,
and, in some cases, it may be implicit (i.e. the interpreter blesses some
values with the \k{matrix} and \k{array} classes even when they have no
attribute). Finally, for each value, the tool outputs the list of
\emph{attribute names} (other than \k{class}).

Depending on the value's type tag, the tool collects further information:

\begin{itemize}
\item For vectors, the number of dimensions and their names are output;
\item For lists, a recursive traversal collects element types and
  the name attributes of elements.  
  TODO: HOW DO WE OUTPUT HETEROGENEOUS NAMES???
\item For promises, we carefully dig into the promise value {\it without
  ...
  without evaluating it and ascribe it type ``any''. 
\end{itemize}
