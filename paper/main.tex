...
\newcommand{\PERCFAILEDASSERTIONSKAGGLENOUNDEF}{0.42\%\xspace}
 ...
 \newcommand{\NUMTOTALASSERTIONSKAGGLE}{1,9038,496\xspace}
...
All vectorized data types have a distinguished missing value denoted by \code{NA}. 
The default type of \code{NA} is \k{logical}. 
We can see that \code{typeof(NA)=="logical"}, but NA inhabits every type: \code{typeof(c(1,NA)[2])=="double"}.
...
Since vectors are unityped in R, it {\it must} have a ``NaN'' value for each primitive type, hence the \code{NA}s, while there is only a single \code{NULL} null-typed value in the language to encompass all other cases.
...
failures, is when a \VEC\dbl is passed where a \CLASS{\texttt{matrix}} is
...
coercion from matrices to vectors is ad hoc at best, and surely not a
practice codified in the language. For instance, if it is determined that we should coerce the matrix it would need to be flattened, and should this be done row-wise or column-wise? 
...
reads: 31 function arguments with \VEC\int type are passed \VEC\dbl instead,
