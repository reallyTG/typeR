...
\AT{NOTE: I am updating this with new numbers as we get them.
The fixed tracer is still running, I should be able to fill in final numbers by tomorrow night.}
...
As for ``unevaled'', first recall the discussion that all function arguments are wrapped in promises in R from \AT{where?}.
The ``unevaled'' signature appears whenever an argument was provided but unused, and one might wonder why such a signature might be so common.
As it happens, many R functions metaprogram on some of their arguments, and this metaprogramming does not force the argument promise, so it will appear as ``unevaled''.
The magrittr package's pipe \%$>$\% function is an example of this.
\AT{Move magrittr discussion up.}
...
Table~\ref{tab:L0funcounts} has the answer.
...
\AT{TODO Replace table with plot in Fig.}

\begin{figure}[htbp]\begin{center}
\includegraphics[width=.9\textwidth]{L0_by_fun}
\caption{Record of Function Polymorphism in {\bf L0}.}
\label{recorded}\end{center}
\end{figure}

Here, we see that the vast majority of functions are monomorphic in L0.
To choosing how to express the polymorphicity of a function, there are two main options one might consider:
...
\begin{itemize}
	...
	This pattern is quite frequent in R code, \AT{and there are a number of examples sprinkled throughout the paper.}
	...
	\AT{Do we want to put the fun def and arg documentation here?}
...
The answer can be found in Table~\ref{tab:funcountsL1}.
...
\caption{Record of Function Polymorphism in {\bf L1}.}
\label{recorded}\end{center}
\end{figure}
...
\AT{Something more about functions. Plots...}
...
%On that note, we wonder: {\it what are most frequent argument signatures with an interesting class?}
%Table~\ref{tab:L2top10argwclass} has the answer.

%% latex table generated in R 3.5.1 by xtable 1.8-3 package
%% Fri Apr  5 02:38:12 2019
%\begin{table}[ht]
%\label{tab:L2top10argwclass}
%\centering
%\begin{tabular}{lllrl}
%  \hline
%Type & Class & Attr Ptrn & Count & \% of All Signatures \\ 
%  \hline
%  \sE & environment & \{\} & 704 & 0.3\% \\ 
%  language & formula & \{.Environment\} & 544 & 0.2\% \\ 
%  \D & array & \{dim\} & 395 & 0.1\% \\ 
%  externalptr & externalptr & \{\} & 301 & 0.1\% \\ 
%  \sF & standardGeneric & \{default,...,srcref,valueClass\} & 255 & 0.1\% \\ 
%  \df & tbl\_df,tbl & \{names,row.names\} & 239 & 0.1\% \\ 
%  \sF & standardGeneric & \{default,...,valueClass\} & 239 & 0.1\% \\ 
%  \l & phylo & \{names,order\} & 199 & 0.1\% \\ 
%  \l & lm & \{names\} & 175 & 0.1\% \\ 
%  \l & lvm & \{names\} & 157 & 0.1\% \\ 
%   \hline
%\end{tabular}
%\caption{Most frequent argument signatures with class in {\bf L2}.}
%\end{table}
%
%Table~\ref{tab:L2top10argwclass} does indeed showcase some interesting classes.
%The three builtin classes which we have not yet seen are environment, formula, and array.
%An environments is a list which has a references to a parent environment (another list with environment class).
%A formula is a \AT{small explanation}.
%An array is a generalization of a matrix in R, where an array can have more than 2 dimensions.
%Each of the other classes pictured here are defined in some package, and are not part of the base language.
%An interesting fact about this table is that all of the pictures signatures are monomorphic.
...
Consider Table~\ref{tab:funcountsL2}.
\AT{This is where the per fun plot should go, will do.}

\begin{table}[ht]
\label{tab:funcountsL2}
\centering
\begin{tabular}{lll}
  \hline
Signature Type & Count & Percentage \\ 
  ...
  Total Seen & 56K & --- \\ 
   \hline
\end{tabular}
\caption{Account of {\it function} polymorphism in {\bf L2}.}
\end{table}

\AT{Connect, talk about increase/decrease in poly.}
