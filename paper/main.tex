...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\subsection{Overview of Types for R}

We ran \typetracer on the \CorpusLoadable packages we discussed in this section, which yields types for \NUMFUNCTIONS functions.
To build up a picture of what types for R functions might look like, consult Table~\ref{tbl:corpus-type-breakdown}, which lists the 10 most popular argument signatures.
The first row of the table reads: 13255 arguments have type \dbl, represnting 11.06\% of type signatures, and \typetracer observed 41M values of type \dbl flowing into or out of functions while running on the core package corpus, which represents only 1.59\% of dynamically observed values.
Of interest are rows 2 and 4, where we see that collectively \lgl and \chr make up only 16.11\% of argument types, while making up nearly 60\% of dynamically observed values.
...
\begin{tabular}{lrrrr}
\toprule
Type & Args with Type & \% of Static Types & Dynamic Observations & \% of Dynamic Types\\
\midrule
double & 13,255 & 11.06 & 41,560,797 & 1.59\\
logical & 10,142 & 8.46 & 712,895,959 & 27.21\\
null & 9,175 & 7.66 & 122,202,183 & 4.66\\
character & 9,171 & 7.65 & 794,058,588 & 30.31\\
double[] & 7,811 & 6.52 & 29,272,232 & 1.12\\
\addlinespace
... & 7,759 & 6.48 & 76,369,333 & 2.92\\
any & 6,243 & 5.21 & 255,539,497 & 9.75\\
character[] & 4,576 & 3.82 & 88,233,523 & 3.37\\
class<`matrix`> & 4,522 & 3.77 & 32,452,555 & 1.24\\
class<`data.frame`> & 2,781 & 2.32 & 7,749,094 & 0.30\\
\bottomrule
\end{tabular}

% END Autogenerated
\label{tbl:corpus-type-breakdown}
...
There are \PercUnitypedPositions monomorphic parameter and return value
positions in function signatures. Only \PercManytypedPositions positions have
four or more types ascribed to them. Parameter PARAMNAME of function
XXXXCOLONCOLONXXX has the maximum, XXX, types ascribed to it. Most functions
admit or return a uni-typed value.
...
only XXX have four or more types ascribed to them.

Figure XXX shows ten most common type categories that occur by
themselves. We observe that three scalar types, double, logical, and
character are most frequent. This is followed by XXX vararg parameter
positions. Next, XX\% of the parameter types are any because arguments at
those positions were either missing or unused in at least one call to those
functions. Together, these types amount to more than 50\%. This is followed
by double vector, the unit type, null, character vector, and classes matrix
...
inclusion of class-like types in our type annotation language. 

%
\paragraph{Data Frames}

One of the most popular classes in R is the \code{data.frame} class.  Data
...
vignette code, obtaining types for package functions, and then subsequently
...
\PERCSUCCFUNNOSTHREE if we count out S3 functions: These S3 functions
...
Table~\ref{tbl:fail-breakdown-0}.  By far the most commonly failed assertion
is one asserting that a value with ${\bf dbl}[]$ type has type ${\bf
  class}\langle matrix \rangle$.  This was a deliberate design decision, as
...
question.  Table~\ref{tbl:fail-breakdown-1} breaks down the results from
Table~\ref{tbl:fail-breakdown-0} by unique parameter position, folding away
multiple identical failed contract assertions for the same parameter
position.  The first row of this table reads: a value of type
\code{double[]} was passed to 15 different function parameters expecting a
matrix, of which there are 1394 in total: 1.08\% of matrix-typed parameters
were passed double vector values.
...
represents many dynamic contract failures, but very few of the integer
vector-typed arguments have their contracts violated by double vector-typed
values.  Rows 5 and 6 are interesting: we see that rather often arguments
expecting time series data are passed double vector or matrix values.  This
...
reads: 44 function arguments with integer vector type are passed double
vectors instead, and 657 arguments have integer vector type.  Had we failed
...
instead.  The most frequent argument type violation pattern in that of data
frames being passed to arguments expecting tibbles: this occurs in only 22\%
