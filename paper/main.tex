...
Figure~\ref{allcloc} shows the size of the code across all the packages we have used. 
The lines of R code are above the X-axis, and lines of C code below.
All counts are $log_{10}$-scaled, and entries are in ascending order by number of lines of R.
Roughly speaking, the majority of R packages have between 100 and 1000 lines of code.
In terms of C, only ~25\% of packages have any C code.
C and C++ are often used to implement parts of a package where performance is critical.

Lastly, Figure~\ref{recorded} plots the relationship between recorded and observed signatures.
Here, \# observed $>$ \# recorded since it's impossible to record more traces than those which pass through the tracer.
Numbers are again $log_{10}$-scaled.

\AT{TODO Touch up graphs.}
\AT{TODO Other interesting plots? Recorded vs \# scripts?}

Having built a sense for the corpus of data that we're dealing with, we will turn our attention to our methods of data collection.
...
\AT{Unused arguments are still picked up by genthat, so we evaluate them.
I suppose this might lead to issues in some rare circumstances.}

\AT{Do we care about this discussion? This is probably a bit too specific to the implementation.}
...
\AT{Note:} My plan for this section is to linearly go through the
