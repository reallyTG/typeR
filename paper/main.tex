...
\AT{Note: some stuff I wrote that might belong somewhere in here, if not,
  delete.}  As mentioned, the syntax is highly unrestrictive to make this
...
well-formdness rules.  Most relevant for our purposes, packages must have
data, examples, vignettes and tests. All code associated with the package
must succesfully run. This means that each of the package in CRAN comes with
...
lines of R code and ??? lines of C code. For each package, we extracted all
...
%% TODO make sure we are analyzing them, and cut the list to ten
...
\tt data.table & Extension of `data.frame`&&& \\
\tt yaml & Convert R data to YAML&&& \\
\tt cli &  Command line interfaces&&& \\
\tt purrr & Functional programming&&& \\\hline
\end{tabular}}
\caption{Most downloaded packages.}\label{most}
\end{figure}
%
%
%
