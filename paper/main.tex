...
\code{data.tree::Traverse}: \FUN{\CLASS{`Node`, `R6`}, ~\VEC\chr, \any, \any}{\LIST{\any}}

\code{openssl::decrypt\_envelope}: \FUN{~\VEC\raw, ~\VEC\raw, ~\VEC\raw, \CLASS{`key`, `rsa`}, \any}~\VEC\raw;

\code{dbplyr:type}: `set\_win\_current\_group` \FUN{? ~\VEC\chr}? ~\VEC\chr;
