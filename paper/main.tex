...
%
%
\subsection{Parametric Types and Higher-Order Function Types}

\AT{Ok, high level:
We don't do higher-order function types, beyond any to any.
Typetracer detected them once, but the data was way too noisy.
We found that mostly these correlated with functions where parametric polymorphism would be suitable, though there's no way to say that from the data, and only manual inspection revealed it.
We could implement some heuristic parametric polymorphism detector, but that would be way unsound.
Ultimately, we felt that higher-order function types and parametric polymorphism were related, and they didn't occur frequently enough across all of R.
Some packages are heavy users, but those packages are heavily metaprogrammed and effectively implement DSLs.
We will revisit this issue, but it's not something that we can pick up with \typetracer as it is, and doing justice to these types will require significant effort.}

In R, functions are first-class values, and they can be passed as function arguments.
