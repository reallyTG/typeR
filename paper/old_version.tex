...
For one, the page limit is way less (12).
This is probably fine. 
I took out the graphs for now, maybe we`ll just want one giant page for all the figures?
We can figure out where to place them.

We want to beef up the contribution.
For this, we should:
\begin{itemize}
\item generate annotations, and turn those annotations to contracts, and check the contracts.
\item try to propagate information from inner functions to outer ones.
\end{itemize}

Refocus on: we analyze nearly all of the existing, readily-available code base for R, and from that analysis we develop a type system for the language.
Can abstract: analyze code for a language to develop a type system for that language.
We use R cause of the tests.

Remove fluff.

Address parametric polymorphism.
...
\item The R Language: R intro, types, introduce tracers

\item The Corpus: CRAN, talk about the code we looked at

\item Methodology: what is the methodology for developing our types, and of testing the packages?

\item Results: what are the results?

\item Related Work: plug related work
...
that would allow users to read data into vectors and call statistical routines
...
\item A type system based on the evidence gathered, fully implemented with
  lightweight, non-intrusive contracts to perform runtime type-checking.
\end{itemize}
...
corpus, we can see the relation between observed calls and recorded signatures in
...
\subsection{PLDI Changes}

We need to make some changes for this submission, as there are now two distinct methodologies.

First, to {\it develop the type system}, we analyzed function calls in package code, including considering package reverse dependencies (i.e. clients of the package) to develop complete type signatures.
\AT{Do we want to do this?} We additionally propagate type information for inner functions to outer ones by connecting known signatures of inner functions to outer functions. 
We previously justify how the R ecosystem is particularly well-suited for this, as package code is curated and packages ship with a suite to showcase package use.

Separately, to {\it test our annotations}, we run our inference tool on package code (incl. aforementioned suite) to synthesize type signatures, turn those signatures into annotations, and turn those annotations into contracts on package functions.
Then, we run the example suites of all client packages to test if the annotations were effective. 
...
have vectors allocated to store them).  We also see that matrices (\M{D})
...
%	One explanation for this is that lists of lists sometimes contain NULL as an element, and this would show up with type \l, again as we do not do any subtyping while computing precise list types.
%	Another explanation is the way R handles object-orientation, which we discuss shortly in Section~\ref{sec:S3S4R5}.
	
	\item \df, \M{D}: in R, it is easy to transform a matrix into a data frame, and this argument is likely a result of that:
